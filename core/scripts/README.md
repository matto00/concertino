...
| `check-pr-mergeable.sh` | Pre-present mergeable check for the orchestrator's Delivery phase (CON-122): auto-reconciles a BEHIND branch once, then requires `mergeStateStatus == CLEAN` before a PR is ever presented as "ready" — human-merge path had no equivalent to `check-merge-readiness.sh`'s condition 2 | `<WORKTREE_PATH> <BRANCH>` |
