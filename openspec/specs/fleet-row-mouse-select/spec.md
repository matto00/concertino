# fleet-row-mouse-select Specification

## Purpose
Lets a user click a run row in the fleet dashboard's row list to select it, via SGR terminal mouse reporting that is enabled and disabled cleanly across every entry/exit path (including crashes), reusing the same selection action the keyboard path already uses.
## Requirements
