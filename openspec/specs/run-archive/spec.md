# run-archive Specification

## Purpose
Defines the fleet-reachable, filterable run-archive screen (`A`): lists every retained run under `.concertino/runs/`, filters live by ticket id/title substring, harness, and startedAt date range, and opens the existing run drill-down for a selected run — reusing the fleet view's own `S.runs` and `matchesQuery`/`rowMatches` predicate rather than a parallel read path.
## Requirements
