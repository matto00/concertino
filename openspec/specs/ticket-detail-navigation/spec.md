# ticket-detail-navigation Specification

## Purpose
Defines the `t` keybinding that opens the full-screen ticket detail view (`ticketview.js`) for the focused/selected row in QUICK START, QUEUED, RUNNING, and DONE, its no-op behavior when no ticket identifier resolves, and how `esc` returns to whichever screen (fleet view or launch pad) the ticket detail view was opened from.
## Requirements
