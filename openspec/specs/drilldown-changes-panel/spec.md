# drilldown-changes-panel Specification

## Purpose
Lets a human see what an agent has actually changed in its worktree while a run is in flight, via a
live CHANGES panel in the drill-down, instead of relying on `tmux attach` or waiting for the PR.
## Requirements
