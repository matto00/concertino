# post-completion-escalation-visibility Specification

## Purpose
Ensures a live escalation raised after a run's `run.end` has already fired (e.g. the orchestrator's one-shot post-cleanup follow-up) renders as `NEEDS YOU` rather than a falsely-stale `DONE` row, for as long as the run's tmux window is still alive to receive an answer.
## Requirements
