# fleet-section-jump Specification

## Purpose
Defines the fleet view's lazygit-style digit-key (`1`-`N`) section jump, numbered positionally over the sections actually rendered this frame rather than a fixed mapping, so the operator can move directly to any on-screen section without walking there one row at a time.
## Requirements
