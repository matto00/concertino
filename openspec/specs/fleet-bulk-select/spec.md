# fleet-bulk-select Specification

## Purpose
Lets an operator mark multiple rows within a section (FAILED, QUEUED) with `space` — a dedicated marker, distinct from the ordinary run/QUEUED-local cursor and persisting across `j`/`k` movement — so that section's existing action key can be applied to the whole selection at once instead of one row at a time.
## Requirements
