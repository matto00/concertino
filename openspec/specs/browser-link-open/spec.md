# browser-link-open Specification

## Purpose
Lets the dashboard TUI open a URL in the operating system's default browser (via `xdg-open` on
Linux), with a visible failure notice instead of a crash when that command is unavailable or
fails.
## Requirements
