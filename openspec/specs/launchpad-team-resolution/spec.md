# launchpad-team-resolution Specification

## Purpose
Defines how the launch pad tells an empty-but-real Linear team apart from a `ticketProvider.teamKey` that matches no team at all, on screen and at `concertino validate` time, since Linear answers both cases with an identical empty result.
## Requirements
