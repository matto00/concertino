...
## Purpose
Defines how an operator marks exactly two DONE runs (from the run-archive screen or fleet's DONE section) and opens a side-by-side compare screen showing both runs' timelines, gate results, and total duration/delta in a narrower, purpose-built two-column rendering.
## Requirements
