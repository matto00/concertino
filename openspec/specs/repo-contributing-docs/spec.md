# repo-contributing-docs Specification

## Purpose
Ensures the repo stays discoverable and self-documenting: a `CONTRIBUTING.md`
covers local dev setup, run/test commands, actual code conventions, and the
`core/` → rendered `scripts/concertino/*` template relationship; a repo-audit
doc tracks structural findings (oversized files, duplicated logic, dead code,
doc drift); and `docs/dashboard.md` stays reconciled with actual UI behavior.
## Requirements
