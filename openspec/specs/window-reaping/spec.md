# window-reaping Specification

## Purpose
Defines the dashboard's conservative auto-reap policy for finished runs' tmux windows: a window is closed only once its run has emitted a terminal `run.end` AND its pane is already dead, scrollback is captured before the kill, and a run that never emitted `run.end` is never touched so a crashed run keeps resolving to `failed` instead of falling through to `unknown`.
## Requirements
