# fleet-view-scroll Specification

## Purpose
Defines how the dashboard's fleet view (`lib/ui/screens/fleet.js`) scrolls its `RUNNING`/`FAILED`/`DONE` sections when selection moves past the currently rendered window, while keeping `NEEDS YOU` pinned and the selection index aligned with the rendered `▸` marker at every scroll offset and terminal height.
## Requirements
