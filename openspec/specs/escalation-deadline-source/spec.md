# escalation-deadline-source Specification

## Purpose
Make `emit-event.sh`'s `CONCERTINO_ESCALATION_TIMEOUT_MIN` actually take effect by sourcing `.concertino.env`, including the fallback needed for the real invocation case (from inside a worktree, whose own copy of that file is never populated), so `--await`'s own deadline lands inside the harness's call-timeout cap instead of always losing the race to a harness-imposed kill.
## Requirements
