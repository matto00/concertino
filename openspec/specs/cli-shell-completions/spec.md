...
## Purpose
Defines which subcommands and flags each generated shell completion script
(`concertino completion <fish|zsh|bash>`) offers, and which flags complete a
fixed value list, so tab-completion stays consistent across all three shells
and every command that takes its own flags.
