# ticket-drafting-escalation Specification

## Purpose
Defines a checkable trigger rule (an enumerated hedge-phrase list plus a structural open-question check) that forces ticket-drafting text to raise a real escalation instead of silently resolving an ambiguous scoping or design fork, and wires it into the orchestrator's one existing ticket-adjacent touchpoint.
## Requirements
