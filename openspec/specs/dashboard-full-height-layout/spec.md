# dashboard-full-height-layout Specification

## Purpose
Defines the grow-to-fill contract for the escalation and launch-plan (confirm-launch) screens — the two screens with no existing capability governing their box layout — so their content box grows to consume the terminal's available rows rather than rendering short of it, mirroring the pattern already established for the fleet, drill-down, and launch-pad screens.
## Requirements
