# launch-presets Specification

## Purpose
Let an operator save a named batch-level harness/speed/provider/agent-merge combination and reapply it to a later launch-plan batch in one keystroke, persisted across dashboard restarts and managed from a dedicated PRESETS screen off the settings screen.
## Requirements
