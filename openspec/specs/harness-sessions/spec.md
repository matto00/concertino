# harness-sessions Specification

## Purpose
Gives the dashboard visibility into every live harness process on the machine — Concertino-managed or freelance — via system-wide process and tmux discovery, so an operator can see and act on sessions the run-centric fleet view alone cannot show.
## Requirements
