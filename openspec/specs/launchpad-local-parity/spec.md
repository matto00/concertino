# launchpad-local-parity Specification

## Purpose
Keeps the launch pad's on-screen copy provider-neutral for a `local`- or `manual`-configured project — a provider-agnostic refresh message and human-readable (rather than raw machine) local ticket status labels.
## Requirements
