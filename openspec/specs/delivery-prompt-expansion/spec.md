# delivery-prompt-expansion Specification

## Purpose
Ensures a non-interactive Codex/OpenCode launch always carries the actual
`/concertino-deliver` instructions in the model's first message, since
neither CLI reliably expands a leading-slash command string from a
non-interactive spawn the way Claude Code does.
## Requirements
