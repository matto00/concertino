# instrumentation-provenance Specification

## Purpose
Records which instrumentation measured a run, and emits a durable event when a consuming repository's rendered scripts drift from what `concertino sync` wrote, so a metric moving because its definition changed is distinguishable from a metric moving because the world changed.. Update Purpose after archive.
