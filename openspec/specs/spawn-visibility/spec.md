# spawn-visibility Specification

## Purpose
Make a dashboard-spawned tmux window visible in the fleet from the instant it is created, rather than only once the launched agent reaches `run.start`, and give a window that never gets that far a distinct, legible "failed to start" signal instead of vanishing.
## Requirements
