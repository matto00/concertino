# fleet-metrics-multi-row-charts Specification

## Purpose
Renders the fleet METRICS panel's throughput chart across multiple stacked
terminal rows in the expanded tier, when there's enough vertical room,
raising its resolution past the single-row `sparkline()`'s 8-level cap.
## Requirements
