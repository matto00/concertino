# tui-liveness-detection Specification

## Purpose
Defines the single, documented signal (`scripts/concertino/tui-attached.sh`, backed by `lib/ui/watch-lock.js`'s PID-liveness pidfile) that answers whether a Concertino TUI is attached to the current run, safe against staleness by always resolving ambiguity toward "not attached."
## Requirements
