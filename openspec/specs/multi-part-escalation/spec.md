# multi-part-escalation Specification

## Purpose
Let an orchestrator raise several genuinely independent sub-questions as one wizard-style escalation, with a structurally checkable completion marker so a partial answer can never be mistaken for a fully resolved one, alongside the existing single-question escalation flow, which stays unchanged.
## Requirements
