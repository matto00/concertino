# design-ticket-type Specification

## Purpose
Lets a ticket be filed honestly as "design" work — acceptance criteria are
"the right escalations got raised and answered," not "code got shipped" —
by detecting it at Setup, extracting and raising its open questions during
Planning, triaging each answer via the existing follow-up-triage
sub-procedure, and defining what "done" and Phase 4 cleanup mean when no
