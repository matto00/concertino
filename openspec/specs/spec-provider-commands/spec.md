# spec-provider-commands Specification

## Purpose
Keeps every openspec command Concertino instructs an agent to run — whether authored in core, injected from config at render time, or scaffolded into a new project — valid against the CLI surface the documentation states it targets, and observably red when planning artifacts are malformed.
## Requirements
