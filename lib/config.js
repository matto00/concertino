...
// source of truth `collectConfigIssues` below and the per-ticket
// `harness:<value>` override validation (CON-62, both `cmdValidate --ticket`
// and `concertino sync`'s CONCERTINO_IMPLEMENTED_HARNESSES env var) both
// check against. Adding a third adapter means updating this list only.
...
function resolveModel(c, harness, role) {
  ...
  return harness === 'codex' ? CODEX_MODEL_FALLBACK : 'sonnet';
}
...
module.exports = {
  REPO, SCHEMA_PATH,
  DEFAULT_MODEL_TIERS, IMPLICIT_DEFAULT_SPEED, ROLES, CODEX_MODEL_FALLBACK, VALID_HARNESSES,
  withModelDefaults, withDefaults, resolveModel,
