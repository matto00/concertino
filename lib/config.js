...
// CON-195: the single pure mapping from a role name to the `{{block:...}}`
// docs-list placeholder name its role template is expected to carry
// (`auditor` -> `docsAuditor`). Consumed by BOTH `lib/cli/render.js`'s
// `docList` switch and `collectConfigIssues`' placeholder-coverage check
// below, so the two can never disagree about what placeholder a role
// expects (design.md Decision 2) — a mapping duplicated in two files is
// exactly the class of drift CON-52 already cost this repo once.
function docsPlaceholderName(role) {
  return 'docs' + role.charAt(0).toUpperCase() + role.slice(1);
}
...
function withDefaults(c) {
  ...
  // CON-166: the planning-artifact ROOT (e.g. `openspec`), distinct from
  // `changeDir` (the per-change directory, e.g. `openspec/changes/<name>`).
  // check-merge-readiness.sh's archive-prefix exclusion needs the root — the
  // archive step also writes under `<root>/specs/**` and
  // `<root>/changes/archive/**`, both siblings of `changeDir`, not under it.
  // Derived from the `/changes/<CHANGE_NAME>` suffix `changeDir` follows by
  // convention; falls back to the first path segment for a `changeDir` that
  // does not follow that convention. Never overridden by a bare truthiness
  // check that would keep a stale value from an earlier config shape.
  if (!sp.changeRoot) {
    const marker = '/changes/<CHANGE_NAME>';
    sp.changeRoot = sp.changeDir.endsWith(marker)
      ? sp.changeDir.slice(0, -marker.length)
      : sp.changeDir.split('/')[0];
  }
  ...
  c.agentMerge = Object.assign(
    { enabled: false, mergeMethod: 'squash', protectedPaths: [] },
    c.agentMerge || {}
  );
  // track-per-run-cost-spend, design.md Decision 3: off by default — the
  // self-maintained pricing table (Decision 2) needs ongoing upkeep a
  // project owner must opt into, not something silently switched on by a
  // bare `concertino sync`.
  c.costTracking = Object.assign(
    { enabled: false },
    c.costTracking || {}
  );
  // CON-148: off by default — `cleanup.sh`'s end-of-run `concertino sync` is
  // the right behavior for a project that gitignores its rendered scripts.
  // A project that TRACKS them needs the opposite: an auto-render at the end
  // of every ticket leaves uncommitted changes in the working tree, and the
  // next ticket's `git add -A` sweeps them into an unrelated PR. That failure
  // is silent and crosses ticket boundaries, so the setting has to live
  // somewhere a re-render cannot erase — which is exactly what routing it
  // through config and `renderEnv` provides.
  c.cleanup = Object.assign(
    { skipSync: false },
    c.cleanup || {}
  );
...
// ---------- cost-tracking hook wiring (track-per-run-cost-spend) -----------
// Runtime counterpart to `mergeCostHookSettings` (lib/cli/emit.js): confirms
// `.claude/settings.json` actually carries the `SessionEnd` AND
// `SubagentStop` hook entries `costTracking.enabled: true` requires — BOTH,
// not just one (design.md Decision 1's empirical finding: `SessionEnd` alone
// only ever reports the orchestrator/root role). Pure JS, unlike
// `checkAgentMergePermission` — that check shells out to a script because
// `core/roles/orchestrator.md` ALSO needs it, pre-auditor-spawn; nothing in
// this change's scope needs the cost-tracking check outside Node, so there is
// no equivalent shared shell script to keep in sync. Returns `null` when the
// check does not apply (costTracking disabled, or claude-code not
// configured) — mirrored by both callers (collectConfigIssues below,
// lib/cli/doctor.js) as "say nothing", the same silent-no-op convention
// checkAgentMergePermission's own callers already follow.
function checkCostTrackingHook(cfg, out) {
  const enabled = !!(cfg.costTracking && cfg.costTracking.enabled);
  const harnesses = Array.isArray(cfg.harnesses) ? cfg.harnesses : [];
  if (!enabled || !harnesses.includes('claude-code')) return null;

  const settingsPath = path.join(out, '.claude', 'settings.json');
  if (!exists(settingsPath)) {
    return { ok: false, reason: 'no .claude/settings.json found' };
  }
  let settings;
  try {
    settings = JSON.parse(read(settingsPath));
  } catch (e) {
    return { ok: false, reason: '.claude/settings.json is not valid JSON' };
  }
  if (settings == null || typeof settings !== 'object') settings = {};

  const hookPresent = (eventName) => {
    const arr = settings.hooks && settings.hooks[eventName];
    if (!Array.isArray(arr)) return false;
    return arr.some((entry) =>
      entry && Array.isArray(entry.hooks) &&
      entry.hooks.some((h) => h && typeof h.command === 'string' && h.command.includes('report-cost.sh')));
  };

  const missing = ['SessionEnd', 'SubagentStop'].filter((name) => !hookPresent(name));
  if (missing.length) {
    return { ok: false, reason: `missing hook entr${missing.length > 1 ? 'ies' : 'y'}: ${missing.join(', ')}` };
  }
  return { ok: true };
}

// Same "already mentions it" defense-in-depth `withAgentMergeFixHint` uses,
// for the sibling hint this check needs.
function withCostTrackingFixHint(reason) {
  return /concertino sync/.test(reason) ? reason : `${reason} — run \`concertino sync\` to add the missing hook(s)`;
}
...
// CON-193 (skeptic-final-2.md, C3 — the SECOND fail-open a denylist let
// through): a `protectedPaths` entry is accepted ONLY if it matches this
// strict POSITIVE pattern for a plain relative glob; everything else is
// rejected BY CONSTRUCTION, with no knowledge of what `!`/`:`/whitespace
// individually "mean" to git. This replaces an earlier
// `entry.startsWith('!') || entry.startsWith(':')` denylist that round 1
// added for the literal shape a review demonstrated (`!record/**`) and
// that round 2's skeptic then defeated with a THIRD shape the denylist
// never enumerated (a leading space: `" !record/**"` starts with neither
// `!` nor `:`, so the denylist accepted it, and git silently matched
// nothing on `:(glob) !record/**` while the diff genuinely touched the
// path). A denylist must enumerate every evasion and is always one shape
// behind; this is why the fix is now a positive accept-pattern instead —
// a future reader who wants to add a new evasion would have to find a
// string this ACCEPT pattern matches (not merely a string some reject
// pattern fails to catch), which is the actual test of "is this really an
// allowlist."
//
// Accepted, and why each rule is here:
//   - first character: `[A-Za-z0-9._-]` only — this alone already excludes
//     every git pathspec magic sigil (`!`, `:`), a leading space, and a
//     leading `/` (absolute path), without this function needing to know
//     any of those are "magic" — they are simply not in the accepted set.
//   - every remaining character: `[A-Za-z0-9._\-/*?\[\]]` — alphanumerics
//     plus the glob/path punctuation `protectedPaths` entries actually
//     need (`.`, `_`, `-`, `/`, `*`, `?`, `[`, `]`). Whitespace, `!`, `:`,
//     and any other pathspec-magic character are simply absent from this
//     set, so a NEW magic shape cannot slip through without a change to
//     this accept pattern itself.
//   - the entry must equal its own `.trim()` — rejected OUTRIGHT (never
//     silently trimmed-and-accepted) if it has leading/trailing
//     whitespace at all, since the char-set rule above already excludes
//     an embedded space but a leading/trailing one is worth naming
//     explicitly in the rejection message, and silent normalization would
//     hide operator error in a safety guard.
//   - no `..` anywhere — blocks a path-escape sequence the char-set rule
//     alone would still allow (`.` and `/` are both accepted characters).
//   - no trailing `/` — the char-set rule alone would still allow a glob
//     ending in `/`; rejected explicitly since a directory-only pathspec
//     has different (and here, unintended) matching semantics.
function isPlainRelativeGlob(entry) {
  if (entry !== entry.trim()) return false;
  if (entry.trim() === '') return false;
  if (entry.includes('..')) return false;
  if (entry.endsWith('/')) return false;
  if (!/^[A-Za-z0-9._-][A-Za-z0-9._\-/*?[\]]*$/.test(entry)) return false;
  return true;
}

// Back-compat wrapper (skeptic-final-1.md's original name) — kept as the
// negation of the allowlist above so every existing caller/test written
// against "is this pathspec magic" keeps working unchanged; the actual
// rejection logic now lives in isPlainRelativeGlob, not here.
function isPathspecMagic(entry) {
  return !isPlainRelativeGlob(entry);
}
...
function collectConfigIssues(cfg, opts) {
  ...
  if (thc) {
    if (thc.kind === 'unsupported-provider') {
      ok('ticket harness', dim(`--ticket live-checking is not supported for ticketProvider.kind "${thc.providerKind}"`));
  ...
  // CON-195: role-template cache for the placeholder-coverage check below —
  // read once per role, from the PACKAGE's own core (REPO, not a resolved
  // worktree core — lib/config.js must not require lib/cli/resolve-core, see
  // design.md Decision 3 / tasks.md 3.3). `undefined` means "could not read
  // this role's template", which is the deliberate fail-silent arm: an
  // unreadable template is an environment fact, not a config defect.
  const roleTemplateCache = new Map();
  const readRoleTemplate = (role) => {
    if (roleTemplateCache.has(role)) return roleTemplateCache.get(role);
    let content;
    try {
      content = read(path.join(REPO, 'core', 'roles', role + '.md'));
    } catch (e) {
      content = undefined;
    }
    roleTemplateCache.set(role, content);
    return content;
  };
  if (!docs.length) {
    ...
    docs.forEach((d, i) => {
      ...
      // CON-195 (design.md Decision 1 / tasks.md 3.1-3.2): every `bindTo`
      // target must (a) be a role this repo actually supports, and (b) have
      // a docs placeholder in its role template to render into — derived
      // from ROLES, not a hardcoded pair, so a future sixth role cannot
      // silently reintroduce this gap. A binding that fails either check is
      // reported here and skipped by the exists()/ok() path below so it is
      // never ALSO reported as successfully configured.
      let bindToOk = true;
      for (const role of d.bindTo) {
        if (!ROLES.includes(role)) {
          fail(`canonicalDocs[${i}].bindTo`,
            `canonicalDocs "${d.path}" binds to unsupported role "${role}" (supported: ${ROLES.join(', ')})`);
          bindToOk = false;
          continue;
        }
        const template = readRoleTemplate(role);
        if (template === undefined) continue; // cannot tell — not a config defect
        const placeholder = '{{block:' + docsPlaceholderName(role) + '}}';
        if (!template.includes(placeholder)) {
          fail(`canonicalDocs[${i}].bindTo`,
            `canonicalDocs "${d.path}" is bound to role "${role}", but core/roles/${role}.md has no ${placeholder} placeholder to render into`);
          bindToOk = false;
        }
      }
      if (!bindToOk) return;
      if (!exists(path.join(out, d.path))) {
  ...
  // CON-148. Checked against the RAW config, not withDefaults()'s output, so a
  // mistyped value is reported rather than masked by the default merge.
  const cleanupConf = cfg.cleanup || {};
  if (cleanupConf.skipSync !== undefined) {
    typeof cleanupConf.skipSync === 'boolean'
      ? ok('cleanup.skipSync', cleanupConf.skipSync)
      : fail('cleanup.skipSync', `cleanup.skipSync must be a boolean (got: ${JSON.stringify(cleanupConf.skipSync)})`);
  }
  ...
  // ── Agent-merge protectedPaths (NEW — CON-193) ──────────────────────────
  // Validated unconditionally (not gated on agentMerge.enabled/claude-code,
  // unlike the permission-grant section below): a malformed entry is a
  // config error regardless of whether agent-merge is currently on, since
  // enabling it later must not silently inherit a bad list. An empty-string
  // entry is its own named arm (not folded into "non-string") because an
  // empty `:(glob)` pathspec matches every path in the tree — see design.md
  // Decision 3.
  if (cfg.agentMerge && cfg.agentMerge.protectedPaths !== undefined) {
    const pp = cfg.agentMerge.protectedPaths;
    if (!Array.isArray(pp)) {
      fail('agentMerge.protectedPaths', `agentMerge.protectedPaths must be an array (got: ${JSON.stringify(pp)})`);
    } else {
      pp.forEach((entry, i) => {
        if (typeof entry !== 'string') {
          fail(`agentMerge.protectedPaths[${i}]`, `agentMerge.protectedPaths[${i}] must be a string (got: ${JSON.stringify(entry)})`);
        } else if (entry === '') {
          fail(`agentMerge.protectedPaths[${i}]`, `agentMerge.protectedPaths[${i}] must not be an empty string — an empty glob pathspec matches every path in the tree`);
        } else if (!isPlainRelativeGlob(entry)) {
          // CON-193 (skeptic-final-2.md, C3): a POSITIVE allowlist test —
          // isPlainRelativeGlob, not a denylist of observed magic prefixes.
          // Round 1's denylist (`startsWith('!')`/`startsWith(':')`) was
          // itself defeated by round 2's leading-space shape
          // (`" !record/**"` starts with neither character); a git
          // pathspec-magic entry, or any other entry that is not
          // recognizably a plain glob, matches NOTHING when handed to git
          // — the check would silently pass even when the diff genuinely
          // touches the path, producing output indistinguishable from a
          // legitimate non-match.
          fail(`agentMerge.protectedPaths[${i}]`, `agentMerge.protectedPaths[${i}] must be a plain relative glob (got: ${JSON.stringify(entry)}) — leading/trailing whitespace, '..', a trailing '/', and any character outside [A-Za-z0-9._-/*?[]] are rejected, since any of those can make git silently match nothing and disarm the guard`);
        }
      });
    }
  }
  ...
  // ── Cost tracking (NEW — track-per-run-cost-spend, design.md Decision 3) ──
  // A genuinely silent no-op — same discipline as the Agent-merge section
  // above — unless costTracking.enabled is true AND claude-code is
  // configured.
  const ctHarnesses = Array.isArray(cfg.harnesses) ? cfg.harnesses : [];
  if (cfg.costTracking && cfg.costTracking.enabled && ctHarnesses.includes('claude-code')) {
    sec('Cost tracking');
    const ctCheck = checkCostTrackingHook(cfg, out);
    if (ctCheck && ctCheck.ok) {
      ok('costTracking.hooks', 'SessionEnd + SubagentStop present in .claude/settings.json');
    } else {
      const reason = (ctCheck && ctCheck.reason) || 'hook entries missing';
      warn('costTracking.hooks', withCostTrackingFixHint(reason));
    }
  }
...
module.exports = {
  ...
  docsPlaceholderName, isPathspecMagic, isPlainRelativeGlob,
  ...
  checkCostTrackingHook, withCostTrackingFixHint,
