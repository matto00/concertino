...
// dashboard's screens (drilldown.js, fleet.js/fleet/sections.js,
// launchpad.js, ticketDetail.js, ticketview.js, ticketdraft.js). See
// openspec/changes/dashboard-iconography-pass/design.md Decision 1/2 and
// shared-widget-layer's design.md Decision 4. Every glyph here is a bare
// string with no colour/SGR of its own — callers wrap with `f.dim`/`f.bold`/
// etc. exactly as they already do for plain text, so icon colour follows the
// same per-call styling as the label it sits next to. Every icon+label
// composition is built via `lib/ui/widgets/header.js`'s `sectionHeader`
// rather than a screen inlining `icon + ' ' + label` independently —
// `docview.js`, `escalation.js`, `settings.js`, and `launchplan.js` route
// their own (icon-less or caller-composed) headers through the same widget
...
const icons = {
  ...
  changes: '◧',     // ◧  U+25E7  CHANGES panel (live git diff against the run's worktree)
};
