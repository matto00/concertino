...
// buildFrame and the escape-sequence constants are exported alongside watch()
// purely for test/watch.test.js (CON-17) — watch() itself runs an interval
// loop against real stdin/stdout, so the pure, escape-sequence-producing
// logic is what gets unit tests; watch()'s own runtime behavior is
// unchanged and still covered end to end by test/scripts/watch-smoke.test.sh.
module.exports = {
  watch, buildFrame, attachAndRestore,
