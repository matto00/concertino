...
async function watch(opts) {
  ...
  await new Promise((resolve) => {
    ...
    function promptKey(key) {
      ...
      if (key === '\x1b' || key === '') { prompt = null; return true; }  // Escape / Ctrl-C
      ...
      // Printable: stale error text from a previous failed submit must not
      // linger once the user starts correcting their input — otherwise it
      // sits there until the next Enter, describing a value that no longer
      // matches what's on screen.
      if (key >= ' ') { prompt.value += key; prompt.error = null; return true; }
      return false;
    }
