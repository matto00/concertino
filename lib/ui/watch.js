...
// this is a no-op for it. Lives in lib/ui/harness.js now (one seam for all
// harness/CLI mapping); re-exported below unchanged for existing callers
// and tests.
...
// Lives in lib/ui/harness.js now — that version also knows opencode (this
// file's old inline copy defaulted an opencode-only project to `claude`).
const defaultLaunchCommand = harnessCmd.defaultLaunchCommand;
...
async function watch(opts) {
  ...
  // `baseCommand` untouched. Every spawn site below routes through this —
  // the queue tick, force-start, the `n` prompt, draft-launch and restart —
  // so a labelled ticket cannot slip through one path and not another. A
  // custom dashboard.launchCommand pins the command outright (same rule as
  // the launch plan's own 'h'/'m' gating: an operator override has no slots
  // to safely rewrite). Reads the ticket cache fresh per spawn, mirroring
  // quickStartEligible()'s own "cheap enough to recompute" precedent — a
  // spawn is far rarer than a frame.
  // Canonical harness ids, straight from config — deliberately NOT the
  // CLI-label-space `configuredHarnesses` local that `open-launchplan`
  // derives for its own display list (that one maps claude-code -> claude).
  const projectHarnesses = (opts.config && Array.isArray(opts.config.harnesses))
    ? opts.config.harnesses : [];
  function commandFor(ticketId, baseCommand) {
    if (cfg.launchCommand) return baseCommand;
  ...
  function draw() {
    ...
    if (queueState && queue.shouldTick(queueState)) {
      ...
      for (const ticket of result.toLaunch) {
        const launched = submitTicket(ticket,
          commandFor(ticket, result.queue.launchCommand || launchCommand), session);
    ...
    // grid mode this frame.
    {
      ...
      // scrollToShow()'s winOpts below.
      const heightOpts = {
        cols: gridCols, rows: computeScreenRows(), selected, scrollOffset, queueState,
        prompt, queueNotice, restoreNotice, quitConfirm, forceStartConfirm, clearQueueConfirm,
      };
      const gridMode = fleetScreen.gridModeEligible(runs, heightOpts);
      ...
      scrollOffset = Math.max(0, Math.min(scrollOffset,
        winFn(runs, Object.assign({}, heightOpts, { rows: 0 })).maxScrollOffset));
    }
  ...
  await new Promise((resolve) => {
    ...
    function scrollToShow(targetSelected) {
      const gridCols = process.stdout.columns || 80;
      // Named `winOpts`, not `opts` — this closure is nested inside
      // `watch(opts)`, and `opts.config`/`opts.root` are read all over that
      // outer closure; a same-named local here would shadow it silently.
      const winOpts = {
        cols: gridCols,
      ...
      // fleet-metrics-grid final-fix (Finding 1): the same height-aware
      // gate as the scrollOffset re-clamp above (fleetScreen.
      // gridModeEligible) — `winOpts.rows` is already the real
      // computeScreenRows() value here, so eligibility and the window
      // itself agree about what "this frame" means.
      const win = fleetScreen.gridModeEligible(runs, winOpts)
        ? fleetScreen.visibleWindowGrid(runs, winOpts)
        : fleetScreen.visibleWindow(runs, winOpts);
    ...
    function applyAction(action) {
      ...
      switch (action.type) {
        ...
        case 'confirm-force-start': {
          ...
          if (queueState) {
            ...
            if (result.toLaunch.length) {
              const launched = submitTicket(action.ticket,
                commandFor(action.ticket, result.queue.launchCommand || launchCommand), session);
              if (!launched.spawned) queueNotice = launched.error;
        ...
        case 'submit-prompt': {
          ...
          // commandFor() gets the BARE id (the typed value may carry a
          // trailing agent-merge/speed token submitTicket itself parses) —
          // an unparseable value degrades to the base command and is then
          // rejected by submitTicket exactly as before.
          const typedId = String(action.value || '').trim().split(/\s+/)[0];
          const result = submitTicket(action.value, commandFor(typedId, launchCommand), session);
        ...
        case 'confirm-draft': {
          ...
          linear.createTicket({ apiKey, teamKey: team.key, title: draft.title, description: body })
            .then((issue) => {
              ...
              // commandFor() for uniformity with every other spawn site —
              // a just-created ticket is not in the cache yet (and carries
              // no labels), so this degrades to the base command today.
              const result = submitTicket(issue.identifier, commandFor(issue.identifier, launchCommand), session);
              ticketDraft = null;
              mode = 'fleet';
              if (!result.spawned) {
        ...
        // (setup-worktree.sh's telemetry), so a ticket launched under codex
        // at `fast` restarts under codex at `fast` — previously it silently
        // came back under the global default harness at default speed.
        // commandFor() still runs on top, so a `harness:` label wins over
        // even the recorded identity, exactly as it would on a fresh launch.
        case 'restart-confirmed': {
          let restartBase = launchCommand;
          if (!cfg.launchCommand) {
            const run = (runs || []).find((r) => r.ticket === action.ticket);
          ...
          const result = control.restartConfirmed(action.ticket, runs, session,
            commandFor(action.ticket, restartBase), submitTicket);
          drillConfirm = null;
        ...
        case 'open-launchplan': {
          ...
          // Per-ticket `harness:<value>` overrides, resolved once here (the
          // same moment ports/commit are snapshotted) in CLI-label space so
          ...
          // cycle). Only tickets that would genuinely dispatch differently
          // at spawn time appear (resolveTicketHarness's own configured/
          // valid filtering) — the row annotation must never promise a
          // harness commandFor() would not actually launch.
          const ticketHarness = {};
          for (const t of tickets) {
            // projectHarnesses (canonical ids), NOT this case's own
            // CLI-label-space configuredHarnesses local just above.
            const th = harnessCmd.resolveTicketHarness(t.labels, projectHarnesses);
          ...
          launchPlan = {
            ...
            launchCommand: cfg.launchCommand || launchplanScreen.withAgentMergeFlag(
              harnessCmd.launchTemplate(harnesses[0]) || launchCommand, agentMerge),
          ...
          if (!cfg.launchCommand) {
            plan.launchCommand = launchplanScreen.withSpeedFlag(
              launchplanScreen.withAgentMergeFlag(
                harnessCmd.launchTemplate(plan.harness) || launchCommand, plan.agentMerge),
              plan.speed);
          }
