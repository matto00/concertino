...
// comment above).
const {
  createSession, hasTmux, attachTarget: attachSessionTarget, killTarget: killSessionTarget,
  // CON-98: the `d` ("mark done") in-process event write (design.md
  // Decision 2) — routed through ctx.deps below, same reasoning as every
  // other session.js export this file requires.
  writeOverrideEvent,
} = require('./session');
...
// CON-111: the launch plan's named batch-level presets store — sibling to
// queue-cache.js, routed through ctx.deps for the same require-cache-fake
// reason every other stateful dependency is (see this file's own header
// comment).
const presetsCache = require('./presets-cache');
...
// CON-104: also changesItems (to clamp drillChangesIndex, mirroring
// evidenceItems above).
const drilldownScreen = require('./screens/drilldown');
const docviewScreen = require('./screens/docview');
// CON-104: the CHANGES panel's own per-poll `git diff --stat` read — see
// lib/ui/git-diff.js's own header comment for why this lives outside any
// pure render path.
...
async function watch(opts) {
  ...
  let running = true;
  // CON-112, design.md Decision 5: the one "has this watch() call already
  // started (or finished) tearing its terminal ownership down" flag, shared
  // by quit() AND the new top-level uncaughtException handler below (see its
  // own comment) — hoisted here, rather than staying a `quit()`-local
  // closure the way it was before this change, specifically so both can
  // read/write the SAME flag and neither can double-write the terminal-
  // restore sequence racing the other.
  let quitting = false;
  ...
  function ensureLaunchPad() {
    if (!S.launchPad) {
      ...
      // CON-44/CON-92: `config` here is normally what lib/cli/watch.js's
      // cmdWatch got back from lib/config.js's withDefaults — but cmdWatch
      // falls back to the raw, un-normalised parsed object (or `{}`) when
      // there's no config file, the JSON fails to parse, or withDefaults
      // itself throws (a parsed config missing the `project`/
      // `ticketProvider` objects it requires — see cmdWatch's own comment).
      // So ticketProvider.kind reaching here can still be absent, typo'd,
      // still the deprecated "manual" (on the fallback path), or "github"
      // (which `concertino validate` accepts but the resolver has no module
      // for) — normalisation doesn't validate `kind` values, it only
      // resolves the one known alias. ticket-provider.js's moduleFor() is deliberately
  ...
  function openLaunchPad() {
    ...
    // Resolved through the provider table (kindFor), not compared against the
    // raw string: cmdWatch normalises via withDefaults on its common path, but
    // falls back to the raw parsed object when normalisation isn't safely
    // possible (see cmdWatch's own comment) — so a project still on the
    // deprecated `manual` can still reach here un-normalised and must get
    // the same local launch pad `concertino validate` promises it.
    if (linear.kindFor(config) === 'local' && S.launchPad.status.enabled) refreshLaunchPad();
  }
  ...
  async function refreshLaunchPad() {
    ...
    try {
      ...
      // Only where the key is load-bearing (ticket-provider.js's
      // NEEDS_TEAM_KEY). For Linear the team key IS the query, so a missing
      // one can only fetch nothing; for local it is inert — fetchTickets
      // scans `tickets/` and merely echoes the key back — so demanding it
      // would hard-fail the smallest schema-valid local config the instant
      // openLaunchPad's auto-refresh fires. A null key degrades instead:
      // launchpad.js's header falls back to a plain "N open", and
      // local.teamNotFoundMessage substitutes "TICKET" for its example id.
      if (!team.key && linear.requiresTeamKey(config)) {
        throw new Error('no ticketProvider.teamKey configured — see config-reference.md');
      }
  ...
  function computeScreenRows() {
    ...
    // escalation's screen" scenario). CON-107: also suppressed on a
    // historical detail view (S.escalationHistoryItem) — see
    // suppressedOnOwnScreen's own header comment.
    const bannerText = bannerScreen.suppressedOnOwnScreen(S.mode, S.escalationTicket, S.liveEscalations, S.escalationHistoryItem)
      ? null
  ...
  // CON-110, tasks.md 3.5: the identical "small, gated, per-poll disk read"
  // draw() itself already builds every frame (just below) — factored out so
  // the fleet controller's 'submit-search' handler (which runs OUTSIDE
  // draw(), at keypress time, needing a QUEUED ticket -> title lookup to
  // resolve a match against a QUEUED row's title) can call the exact same
  // thing rather than a second, potentially-stale implementation. Mirrors
  // quickStartEligible's own "recomputed fresh on every call" precedent just
  // above.
  // CON-107, design.md Decision 1: the full, unbounded, newest-first
  // raised/resolved escalation history metricsFor() now computes — re-
  // derived fresh on every call (never cached across draws), the same
  // "never trust a value from a previous draw()" discipline
  // quickStartEligible already established, so the fleet controller's
  // move-metrics-focus/open-historical-escalation handlers (and draw()'s own
  // re-clamp, below) can never disagree about what the current history
  // actually is.
  function metricsEscalationHistory() {
    return fleetScreen.metricsFor(S.runs, Date.now()).recentEscalations;
  }

  function queuedTitlesFor() {
    return (S.queueState && S.queueState.pending.length)
  ...
  const ctx = {
    ...
    deps: {
      store, linear, draftHelper, control, cache, queue, queueCache, presetsCache, submitTicket,
      ...
      // CON-98: the `d` ("mark done") in-process event write — same
      // fakeable-dependency reasoning as every other entry here.
      writeOverrideEvent,
    },
    computeScreenRows,
    quickStartEligible,
    metricsEscalationHistory,
    queuedTitles: queuedTitlesFor,
  ...
  function draw() {
    ...
    // grid mode this frame.
    {
      ...
      // forceStartConfirm/clearQueueConfirm/markDoneConfirm) that
      // buildHeadTail() reads — omitting them makes this opts object
      // systematically OVER-estimate columnAreaHeight relative to what
      // renderFleet actually computes (which always receives the full render
      // opts), so this clamp could pick grid mode's maxScrollOffset while
      // the renderer draws single-column, or vice versa. Same fields, same
      // reasoning, as the fleet controller's scrollToShow winOpts.
      const heightOpts = {
        ...
        // CON-98: markDoneConfirm's banner (confirmLines(), same shape as
        // forceStartConfirm's) also lengthens buildHeadTail()'s tail.
        ...
        // lengthen buildHeadTail()'s tail exactly like markDoneConfirm's
        // just above — same fix, same reasoning, and bulkResult in
        // particular can render several lines (one per ticket), so omitting
        // it here is even more likely to visibly desync scrolling than the
        // historical markDoneConfirm omission this mirrors.
        bulkConfirm: S.bulkConfirm, bulkResult: S.bulkResult,
        // CON-114, skeptic gate (final, round 1): compareSelection now
        // feeds a conditional `c compare` hint (and can turn on
        // `space select` with no FAILED/QUEUED section present) — same
        // "every tail-lengthening field must be threaded here too"
        // discipline as bulkConfirm/bulkResult just above, same reasoning.
    ...
    // CON-107, design.md Decision 2: the METRICS-local escalation-history
    // cursor's own re-clamp, same discipline as quickStartFocus's just
    // above — the underlying history array's length can shift between polls
    // (a newly-raised escalation shifts every existing index by one, since
    // the list is newest-first), so a stale, out-of-range cursor must never
    // survive to the next draw.
    if (S.focus === 'metrics') {
      const len = metricsEscalationHistory().length;
      if (S.metricsEscalationFocus == null || S.metricsEscalationFocus < 0 || S.metricsEscalationFocus >= len) {
        S.metricsEscalationFocus = 0;
      }
    }
    ...
    //
    // CON-107, design.md Decision 4 (skeptic round 1 correction): exempted
    // while `S.escalationHistoryItem` is set — a historical view carries
    // nothing that can change out from under it (a static snapshot of an
    // already-resolved entry, looked up by no ticket at all —
    // `S.escalationTicket` stays `null` for it, per Decision 5), so without
    // this guard this check would find no matching run and bounce every
    // historical view back to the fleet on the very next poll (~1s cadence).
    ...
    if (S.mode === 'drilldown' && S.drillTicket) {
      ...
      // CON-104 (design.md Decision 1/2): the CHANGES panel's `git diff
      // --stat`, recomputed every poll while the drill-down is open on this
      // ticket — gated to exactly this one run, never the whole fleet, and
      // never for a run not currently being viewed (spec.md's own
      // requirement). `null` is the sentinel for "no worktree to diff"
      // (unset, or removed by cleanup.sh --phase4) — checked with a cheap
      // fs.existsSync BEFORE ever shelling out to git, so a gone worktree
      // costs nothing beyond that one stat call.
      S.drillDiffStat = (run && run.worktree && fs.existsSync(run.worktree))
        ? gitDiff.diffStat(run.worktree)
        : null;
      const changesFiles = drilldownScreen.changesItems(S.drillDiffStat);
      S.drillChangesIndex = Math.max(0, Math.min(S.drillChangesIndex, Math.max(0, changesFiles.length - 1)));
    }
    if (S.mode === 'docview') {
    ...
    // CON-110: now the same `queuedTitlesFor()` ctx.queuedTitles calls too
    // (tasks.md 3.5) — one implementation, never two that could drift.
    const queuedTitles = queuedTitlesFor();
    ...
    // CON-112, design.md Decision 3: the fleet row-index map for THIS
    // frame's click hit-testing — recomputed every draw() alongside
    // screenText itself (never cached across frames, and rebuilt from the
    // exact same opts router.render just used), so the two can never
    // disagree about what "this frame" rendered. `null` outside fleet mode
    // — nothing to click there this pass (design.md's explicit scope
    // decision) — so a click while any other screen is on top always
    // resolves to a no-op (see onKey's own click handling below).
    if (S.mode === 'fleet') {
      const localRowMap = fleetScreen.renderRowMap(currentState(), {
        cols, rows: screenRows, now, queuedTitles, ticketText: drillTicketText,
        quickStartTickets, quickStartCold,
      });
      // `rowMap`'s keys are 0-based WITHIN screenText itself — `rendered`
      // (just below) prefixes it with exactly one top-bar line plus
      // `bannerLines` banner lines, so a terminal row maps to a screenText
      // line at `terminalRow - (1 + bannerLines) - 1`, i.e. screenText line
      // `li` sits at absolute terminal row `li + 2 + bannerLines` — the same
      // "+1 topbar, +1 for the line right after the banner" arithmetic
      // `rendered`'s own template literal encodes structurally.
      const bannerLines = bannerText ? bannerText.split('\n').length : 0;
      const rowOffset = 2 + bannerLines;
      const absoluteRowMap = {};
      for (const key of Object.keys(localRowMap)) absoluteRowMap[Number(key) + rowOffset] = localRowMap[key];
      S.fleetRowMap = absoluteRowMap;
    } else {
      S.fleetRowMap = null;
    }
  ...
  // CON-112, design.md Decision 1: MOUSE_REPORT_ENTER is written at exactly
  // the same point (same `stdin.isTTY` guard) `setRawMode(true)` itself is —
  // a non-TTY stdin (piped input, the smoke gate's redirected stdout) gets
  // neither, exactly as before this change.
  if (stdin.isTTY) { stdin.setRawMode(true); process.stdout.write(MOUSE_REPORT_ENTER); }
  stdin.resume();
  stdin.setEncoding('utf8');

  // CON-112, design.md Decision 5: a genuine top-level uncaught-exception
  // handler — none existed before this change (see design.md's own
  // ground-truth correction: an uncaught throw from onKey/applyAction/the
  // poll timer's draw() callback previously skipped quit()'s terminal
  // restore entirely, leaving the user's terminal in raw mode, the
  // alternate screen buffer, and now mouse-reporting mode too). Restores
  // the FULL terminal state — not mouse-reporting alone — then surfaces the
  // error and exits, rather than silently swallowing it. Guarded by the
  // same `quitting` re-entrancy flag `quit()` uses (declared above this
  // function, shared by both) so a throw racing an already-in-progress/
  // complete shutdown can never double-write the restore sequence.
  // Removes itself (in addition to `quit()`'s own removal — see its
  // comment) before exiting: `process.exit()` never returns in production,
  // so this is a belt-and-braces duplicate there (removal happens exactly
  // once either way, since both are gated on the same `quitting` flip), but
  // it is what keeps this listener from outliving a single `watch()` call
  // under any test harness that fakes `process.exit` to observe this path
  ...
  function onUncaughtException(err) {
    ...
    process.stdout.write(ALT_SCREEN_EXIT + MOUSE_REPORT_EXIT + CURSOR_SHOW);
  ...
  await new Promise((resolve) => {
    ...
    const quit = () => {
      ...
      if (quitting) return;
      quitting = true;
      // CON-112, design.md Decision 5's listener-lifecycle note: removed
      // here, exactly once (guarded by the `quitting` check just above),
      // so a later `watch()` call in the same process starts with a clean
      // slate rather than accumulating one stale handler per call — unlike
      // the pre-existing, never-removed `process.stdout.on('resize', ...)`
      // listener (a separate, already-accepted trade-off this change does
      // not touch).
      process.removeListener('uncaughtException', onUncaughtException);
      clearInterval(timer);
      if (stdin.isTTY) { stdin.setRawMode(false); process.stdout.write(MOUSE_REPORT_EXIT); }
      stdin.pause();
    ...
    function doAttach(ticket) {
      ...
      process.stdout.write(ALT_SCREEN_EXIT + CURSOR_SHOW);
      running = false;
      // CON-112, design.md Decision 1: mouse reporting is disabled before
      // handing the terminal to tmux — tmux runs its own mouse mode inside
      // the attached pane, and this dashboard's own `?1000`/`?1006` state
      // must not still be active underneath it.
      if (stdin.isTTY) { stdin.setRawMode(false); process.stdout.write(MOUSE_REPORT_EXIT); }
      stdin.pause();
      ...
      attachAndRestore(() => session.attach(ticket), () => {
        process.stdout.write(ALT_SCREEN_ENTER + CURSOR_HIDE);
        // CON-112: mouse reporting comes back in the same restore pass as
        // raw mode itself, matching the alt-screen re-entry line just above.
        if (stdin.isTTY) { stdin.setRawMode(true); process.stdout.write(MOUSE_REPORT_ENTER); }
        stdin.resume();
        running = true;
    ...
    function doAttachTarget(sessionName, windowId) {
      process.stdout.write(ALT_SCREEN_EXIT + CURSOR_SHOW);
      running = false;
      // CON-112, design.md Decision 1: same suspend/restore pairing as
      // doAttach() above, applied to this freelance-attach twin.
      if (stdin.isTTY) { stdin.setRawMode(false); process.stdout.write(MOUSE_REPORT_EXIT); }
      stdin.pause();
    ...
    function onKey(key) {
      // CON-109, design.md Decision 4 (skeptic gate round 1, finding 3;
      // evaluator cycle-1 non-blocking suggestion): the ONE place
      // S.bulkResult is ever cleared — at the very top of onKey, before
      // EVERY other branch below (the mouse-click intercept, the banner
      // reply box, the reserved-g-key banner-open, and the ordinary
      // router.handleKey path), not only immediately before
      // router.handleKey. A mouse click or `g` press while a bulkResult
      // banner is visible must dismiss it too, exactly like an ordinary
      // keypress does — leaving those two early-return branches unguarded
      // would let a click or `g` leave the notice on screen stale until the
      // next key that happens to reach router.handleKey.
      if (S.bulkResult) S.bulkResult = null;

      // CON-112, design.md Decisions 2/4: an SGR mouse sequence is recognized
      // and handled BEFORE any of this function's own key-routing (the reply
      // box guard and the reserved-key guard just below, and router.handleKey
      // itself) — text-entry click support is explicitly out of scope this
      // pass (design.md Non-Goals), so intercepting it here is what keeps a
      // click from ever being typed as literal garbage characters into an
      // open reply box/prompt, which falling through to any of those paths
      // would otherwise do. Only meaningful in fleet mode, and only when the
      // click's row resolves against THIS frame's row map (draw(), above) —
      // anything else (any other screen, a click outside the rendered row
      // list) is a silent no-op, exactly like the spec's own "click outside
      // any rendered row is a no-op" scenario. `parseMouseClick` returns
      // `null` for a release event, any non-left-button event, or an
      // ordinary (non-mouse) CSI sequence, all of which fall through
      // unchanged to the ordinary keypress path below.
      const click = parseMouseClick(key);
      if (click) {
        if (S.mode === 'fleet' && S.fleetRowMap &&
            Object.prototype.hasOwnProperty.call(S.fleetRowMap, click.row)) {
          if (applyAction({ type: 'jump', index: S.fleetRowMap[click.row] })) S.runs = draw();
        }
        return;
      }
