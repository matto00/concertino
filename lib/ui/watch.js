...
// CON-20: the exact wording `refreshLaunchPad` and `ensureLaunchPad` both
// need — a live refresh that just confirmed the team doesn't resolve, and a
// fresh process rebuilding that same conclusion from a persisted cache row,
// respectively (see ensureLaunchPad's own comment for why the latter exists
// at all). One function so the two call sites can never drift apart in
// wording.
...
async function watch(opts) {
  ...
  const ctx = {
    ...
    deps: { store, linear, draftHelper, control, cache, queue, queueCache, submitTicket },
    computeScreenRows,
    quickStartEligible,
    resolveModels: (speed, harness) => resolveModelsForPlan(root, speed, harness),
    openInBrowser,
    ensureLaunchPad,
    openLaunchPad,
    refreshLaunchPad,
  ...
  function draw() {
    ...
    if (S.queueState && queue.shouldTick(S.queueState)) {
      ...
      for (const ticket of result.toLaunch) {
        const launched = launcher.launch(ticket, result.queue.launchCommand);
    ...
    reap.reapFinished(root, session, S.runs);
  ...
  S.runs = draw();
  const timer = setInterval(() => { if (running) S.runs = draw(); }, POLL_MS);
  ...
  await new Promise((resolve) => {
    ...
    // (state, opts) -> string. The two cases kept inline here are the two
    // that reach into this closure's own terminal/loop internals: 'back' is
    // pure navigation shared by several screens, and 'attach' hands the
    // terminal itself away.
