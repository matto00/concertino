...
// the terminal shows neither frame. `buildFrame` strips exactly one trailing
// '\n' before splitting into lines (CON-26), so `draw()`'s appended newline
// never contributes an extra row — the line count this function pads from
// and the line count it returns remain in one-to-one correspondence with
// actual rendered content (design.md Decision 2). Padding reuses format.js's
// own visible-column-width `padTo`, not a raw `.length` scheme — outer-frame
// lines carry ANSI SGR colour escapes (f.bold/f.dim/f.yellow/...) that
// `.length` would wrongly count as columns they do not occupy.
//
...
async function watch(opts) {
  ...
  // this block.
  {
    ...
    if (queueRecord && !queueCache.isStale(queueRecord, startupNow)) {
      const restored = queue.createRestoredQueue(queueRecord, startupRuns);
    ...
    function applyAction(action) {
      ...
      switch (action.type) {
        case 'move': {
          ...
          // If the move just placed `selected` outside the currently
          // rendered window, scroll by exactly the overshoot so it becomes
          // the new edge row — never further (design.md Decision 3). Uses
          // the same visibleWindow() the renderer itself calls, so this
          // decision and the next draw()'s actual render can never disagree
          // about what "visible" means.
          const win = fleetScreen.visibleWindow(runs, {
            cols: process.stdout.columns || 80,
            rows: computeScreenRows(),
            selected, scrollOffset, prompt, queueNotice, queueState, quitConfirm,
          });
          if (selected < win.firstVisibleIndex) {
            scrollOffset = Math.max(0, scrollOffset - (win.firstVisibleIndex - selected));
          } else if (selected > win.lastVisibleIndex) {
            scrollOffset = Math.min(win.maxScrollOffset, scrollOffset + (selected - win.lastVisibleIndex));
          }
          return true;
        }
