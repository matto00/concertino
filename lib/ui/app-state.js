...
function createAppState() {
  return {
    ...
    scrollOffset: 0,
    // CON-112: `{ [terminalRow]: runsIndex }` for the fleet screen's own
    // rendered run rows THIS frame — rebuilt by watch.js's draw() on every
    // poll (never reused across frames), `null` outside fleet mode. Not part
    // of `currentState()`'s own snapshot below (screens never read this —
    // only watch.js's onKey does, to resolve a click) but lives alongside
    // the rest of the loop's per-frame state for the same reason `runs`
    // itself does.
    fleetRowMap: null,
    prompt: null,                // null, or { value, error } while `n` is open
    // CON-110: the fleet view's own `/` search prompt — `null`, or
    // `{ value }` while open (structurally `prompt`'s own shape, minus
    // `error`: a free-text search query has no invalid-input case the way
    // the `n` prompt's ticket-shape validation does — design.md Decision 4).
    // Mutually exclusive with `prompt` (`open-search`/`open-prompt` each
    // only fire when neither the other nor any confirmation gate is already
    // open — see fleet/keys.js's handleKey).
    search: null,
    ...
    // CON-107, design.md Decision 4: the paired-history entry (design.md
    // Decision 1's shape) a RESOLVED (or timed-out) METRICS row was opened
    // with — `null` whenever the escalation screen shows a genuinely live
    // escalation (or is not on screen at all). Never aliased to
    // `escalationTicket` — a historical entry needs no live run at all.
    // Reset alongside escalationTicket/escalationReply/... (backToFleet(),
    // below) and defensively at the top of 'open-escalation' itself
    // (controllers/escalation.js), so a stale value can never leak into a
    // later, unrelated (or genuinely live) escalation screen.
    escalationHistoryItem: null,
    ...
    // CON-19, extended by the lazygit-layout pass's five-panel focus model
    // (drilldown.js's DRILL_PANELS; CHANGES added by CON-104): `drillFocus`
    // is always one of 'ticket' (default — the first panel), 'timeline',
    // 'gates', 'evidence', or 'changes' — never null. Digit keys 1-5 jump
    // directly; \t cycles through all five.
    ...
    // CON-104: mirrors drillEvidenceIndex above, for the CHANGES panel's own
    // diff-stat selection — meaningful only while `drillFocus === 'changes'`,
    // kept around the same way while focus is elsewhere.
    drillChangesIndex: 0,
    // CON-104: the CHANGES panel's own per-poll `git diff --stat` read
    // (watch.js's draw(), gated to the ticket currently drilled into) —
    // `null` is the sentinel for "no worktree to diff" (see
    // drilldown.js's changesLines()); otherwise `{ stat, error }` from
    // lib/ui/git-diff.js's diffStat(). Lives on S (not just threaded through
    // `opts` the way ticketText is) because routeHandleKey also needs the
    // same file list to resolve the selected file's path when `↵` opens it —
    // see drilldown.js's render()/routeHandleKey() and screens/drilldown.js's
    // own handleKey.
    drillDiffStat: null,
    ...
    // already uses for tickets (cache.js/tickets.json) — and removed once the
    ...
    // this to model.
    quickStartFocus: 0,
    // CON-107, design.md Decision 2: mirrors quickStartFocus's own shape and
    // relationship to `focus` exactly — an index into the full raised/
    // resolved escalation history metricsFor() computes, meaningful only
    // while `focus === 'metrics'`. Defaults to `0` for the same reason
    // quickStartFocus does: METRICS is on screen from the first frame.
    metricsEscalationFocus: 0,
    ...
    // CON-98, design.md Decision 2: `d`'s ("mark done") own y/anything-else
    // confirmation gate — `{ ticket } | null`, mirroring forceStartConfirm's
    // shape exactly (see fleet.js's handleKey, which checks this alongside
    // forceStartConfirm/clearQueueConfirm, and sections.js's buildHeadTail,
    // which renders its on-screen banner the same way theirs already is).
    markDoneConfirm: null,
    // CON-98, design.md Decision 3: `a`'s own inline notice — set instead of
    // spawning when the selected FAILED run's harness is not claude-code
    // (Non-Goals). Deliberately its own field rather than overloading
    // drillNotice (fleet-screen-scoped, not drilldown-scoped) or queueNotice
    // (a different concern entirely).
    addressFailureNotice: null,
    ...
    // per-visit state.
    settings: null,
    // CON-111: the PRESETS screen's own session state (mode = 'presets').
    // `null` until `p` is first pressed from the settings screen; rebuilt
    // FRESH by the presets controller's openPresets() every time the screen
    // is entered (design.md Decision 6, mirroring `settings`'s own "no
    // persistent reuse across visits" discipline just above). Cleared back
    // to `null` on Escape (discard) and on a successful `S` (save) — never
    // by backToFleet() alone reaching this screen directly, since mode =
    // 'presets' is only ever entered from 'settings' — but reset there too,
    // defensively, same discipline as settings/ticketDraft/sessions* above.
    presets: null,
    ...
    sessionsError: null,
    // CON-113: the run-archive screen's own state (mode = 'archive'). `null`
    // until `A` is first pressed (mirrors settings'/sessions' own "null
    // until opened" discipline) — rebuilt fresh every time `open-archive`
    // runs (design.md Decision 3: no cross-visit reuse, unlike launchPad).
    // `archiveFocus` defaults to `'query'` (the substring input, per
    // spec.md's "defaulting to the substring input when the screen opens"),
    // `archiveDatePrompt` to `null` (no date prompt open), and the rest to
    // null/0 as sessions.js's own fields do. `archiveSelected` is a cursor
    // index into the CURRENTLY FILTERED list, not `runs` itself — see
    // screens/archive.js's filterArchiveRuns.
    archiveQuery: null,
    ...
    // CON-109, design.md Decision 1: per-section multi-select sets, keyed by
    // ticket id (not row index — mirrors how FAILED/QUEUED's existing
    // single-row actions already resolve `ticket` before dispatching, so a
    // row's position shifting between frames never desyncs the selection).
    // Two independent Sets, never one shared set with a section tag, so
    // clearing one on section-exit (Decision 3) can never touch the other.
    multiSelect: { failed: new Set(), queued: new Set() },
    // `{ section, kind, tickets } | null` — the bulk equivalent of
    // markDoneConfirm/forceStartConfirm's y/anything-else confirmation gate,
    // naming a whole ticket list instead of one ticket (design.md Decision 4).
    bulkConfirm: null,
    // `{ kind, results: [{ ticket, ok, error }] } | null` — the one-shot,
    // per-row result list rendered after a bulk confirmation resolves.
    // Cleared on the very next keypress (watch.js's onKey, NOT a
    // fleet/keys.js intercept — design.md Decision 4 / skeptic gate round 1,
    // finding 3), so the key that dismisses it still performs its own
    // ordinary action too.
    bulkResult: null,
    // CON-114, design.md Decision 1: the run-comparison feature's own
    // capped-at-2 selection, shared across the archive screen and fleet's
    // DONE section (lib/ui/compare-selection.js owns the toggle/cap
    // semantics) — an array of up to 2 ticket ids, insertion order
    // preserved for consistent left/right column assignment
    // (screens/compare.js). Deliberately NOT reset by backToFleet() or on
    // entry/exit of the compare screen itself (design.md's "Selection
    // lifecycle, precisely") — it changes ONLY via an explicit
    // toggle-compare-select action.
    compareSelection: [],
    // Which screen ('archive' | 'fleet') the compare screen was most
    // recently opened from — mirrors ticketviewReturnMode's own pattern
    // exactly (design.md Decision 3). Consumed (and reset to null) by
    // 'back-to-origin-from-compare'.
    compareReturnMode: null,
    // Each column's own free-scroll offset (design.md Decision 2 / tasks.md
    // 4.6) and which column ('left' | 'right') arrow/j-k keys currently
    // scroll while the compare screen is open — all three reset on both
    // entry (open-compare) and exit (back-to-origin-from-compare), unlike
    // compareSelection itself, which persists.
    compareLeftScroll: 0,
...
function currentState(S) {
  return {
    ...
    prompt: S.prompt, search: S.search,
    escalationTicket: S.escalationTicket, escalationReply: S.escalationReply,
    ...
    // CON-107: the read-only historical-view entry (design.md Decision 5) —
    // read straight off `state`, same pattern as escalationTicket just above.
    escalationHistoryItem: S.escalationHistoryItem,
    ...
    drillChangesIndex: S.drillChangesIndex, drillDiffStat: S.drillDiffStat,
    ...
    // CON-107: the METRICS-local escalation-history cursor — same
    // "read straight off state" pattern as quickStartFocus just above.
    metricsEscalationFocus: S.metricsEscalationFocus,
    markDoneConfirm: S.markDoneConfirm, addressFailureNotice: S.addressFailureNotice,
    multiSelect: S.multiSelect, bulkConfirm: S.bulkConfirm, bulkResult: S.bulkResult,
    ...
    // CON-113: the run-archive screen's own filter/focus/cursor state.
    archiveQuery: S.archiveQuery, archiveHarnessFilter: S.archiveHarnessFilter,
    archiveDateFrom: S.archiveDateFrom, archiveDateTo: S.archiveDateTo,
    archiveSelected: S.archiveSelected, archiveFocus: S.archiveFocus,
    archiveDatePrompt: S.archiveDatePrompt,
    // CON-114: the run-comparison feature's shared selection and the
    // compare screen's own transient view state.
    compareSelection: S.compareSelection, compareReturnMode: S.compareReturnMode,
    compareLeftScroll: S.compareLeftScroll, compareRightScroll: S.compareRightScroll,
    compareFocus: S.compareFocus,
  };
}
...
function backToFleet(S) {
  ...
  // CON-107, design.md Decision 4's round-2 correction: a field that is only
  // ever SET and never explicitly CLEARED goes stale the moment the operator
  // opens a resolved entry, backs out, then opens a still-live one — cleared
  // here too, alongside every other escalation-screen-local field just
  // above, means it can never leak into a later, unrelated screen.
  ...
  S.settings = null;
  // CON-111: mode = 'presets' is only ever entered from 'settings' (never
  // straight from 'fleet' — see screens/presets.js's own header comment),
  // but cleared here too, defensively, same discipline as settings/
  // ticketDraft/sessions* above — a leaked staged-but-undiscarded session
  // must never survive into a later, unrelated screen.
  S.presets = null;
  ...
  // CON-113: mode = 'archive' always routes back to 'fleet' too (Escape,
  // with no prompt open, is its only exit — see screens/archive.js's own
  // handleKey) — cleared here so a leaked filter/focus/prompt session can
  // never survive into a later, unrelated screen (same discipline as
  // settings/ticketDraft/sessions* above).
  S.archiveQuery = null;
  ...
  // CON-114: defensive reset, same discipline as ticketviewReturnMode/
  // archive* just above — 'back-to-origin-from-compare' already resets
  // these on every consumption, but a leaked value must never survive into
  // an unrelated screen either way. S.compareSelection is deliberately NOT
  // reset here — design.md's "Selection lifecycle, precisely": it persists
  // across trips through fleet/archive/back, changing only via
