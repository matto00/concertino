...
function createLauncher({ root, session, cfg, config }) {
  ...
  function specFor(ticketId, baseCommand) {
    ...
    const spec = harnessCmd.launchSpecForTicket(t && t.labels, baseCommand, fullConfig);
    // Name the session after the ticket it is delivering. The cache read
    // above already has the title in hand, and this is the only layer that
    // does — harness.js is pure and the launch plan builds commands before
    // a ticket is even chosen. A ticket missing from the cache still gets
    // its id, which beats an auto-generated name either way.
    spec.command = harnessCmd.withSessionName(
      spec.command, harnessCmd.sessionNameFor(ticketId, t && t.title));
    return spec;
  }
