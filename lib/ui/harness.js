...
// Codex takes its provider from the command line, not from env — and it has
// FIRST-CLASS local-model flags, which is the only route that actually
// works. Verified against codex-cli 0.146.0:
//
//   --oss                       use the open-source provider
//   --local-provider <ollama|lmstudio>
//   -m <MODEL>                  the model that provider should run
//
// This replaces CON-65's original `-c model_provider=ollama`, which was a
// real bug, not a stylistic choice: `model_providers` cannot be defined in a
// project-local `.codex/config.toml` (Codex ignores the key and says so on
// every launch), so that override named a provider Codex could not resolve.
// The run then died on a malformed request — `input[0]: unknown input item
// type: "additional_tools"` — after silently falling back to the hosted
// default model. Reproduced directly, and fixed by using the supported
// flags instead; see lib/cli/emit.js's codexOllamaConfigToml.
//
// `-m` is passed explicitly because a bare `--oss` prompts for a model
// interactively, which would hang a spawned tmux window with no one typing
// into it. The model is the ORCHESTRATOR's — the top-level Codex session
// plays that role, and on Codex the whole workflow runs sequentially in
// that one thread (see adapters/codex/prompt.md).
//
// Nothing is emitted for 'default': Codex's own default provider needs no
// flag, and naming one is exactly what broke this before.
//
// NOTE: Codex's --oss path requires a THINKING-capable model (it errors with
// `"<model>" does not support thinking` otherwise). `concertino doctor`
// checks this — see CON-73.
function providerCommandFlags(provider, harness, config) {
  if (!provider || canonicalHarness(harness) !== 'codex') return '';
  if (provider !== 'ollama') return '';
  const ollama = (config && config.providers && config.providers.ollama) || {};
  const model = ollama.models && ollama.models.orchestrator;
  return '--oss --local-provider ollama' + (model ? ' -m ' + model : '');
}

// CON-69: the provider values the launch plan's per-row 'P' cycle may offer
// for a given harness — null (follow the batch/label default) always;
// 'ollama' when the provider is configured and the harness can actually
// reach it (claude-code needs the gateway); 'default' only when the project
// routes this harness to Ollama by default, so there is something to flip
// back FROM. The same validity rules resolveTicketProvider applies to
// labels, expressed as a choice list for the UI.
function providerChoices(config, harness) {
  const ollama = config && config.providers && config.providers.ollama;
  if (!ollama) return [null];
  const h = canonicalHarness(harness);
  const choices = [null];
  if (h !== 'claude-code' || (ollama.gateway && ollama.gateway.baseUrl)) choices.push('ollama');
  if ((ollama.harnesses || []).includes(h)) choices.push('default');
  return choices;
}

// CON-69: builds the concrete spawn spec for an explicit set of per-row
// choices — the launch plan's confirm bakes one of these into the queue
// record for every row whose effective (harness, speed, provider) differs
// from the batch, so the queue tick can spawn it verbatim later without
// re-deriving anything. Mirrors the batch command's own construction
// (cycle-harness's withAgentMergeFlag + withSpeedFlag order) exactly, then
// applies the same provider decoration launchSpecForTicket does. null when
// the harness has no template (an unvalidated config naming an
// unimplemented harness must not bake a broken command into a durable
// queue record).
function launchSpecForChoices(choices, config) {
  const tmpl = launchTemplate(choices.harness);
  if (!tmpl) return null;
  let cmd = withSpeedFlag(withAgentMergeFlag(tmpl, !!choices.agentMerge), choices.speed || 'default');
  let env = null;
  if (choices.provider) {
    const flags = providerCommandFlags(choices.provider, choices.harness, config);
    if (flags) cmd = cmd.replace(/^(\S+)\s/, '$1 ' + flags + ' ');
    env = providerSpawnEnv(choices.provider, config, choices.harness);
  }
...
function launchSpecForTicket(labels, batchCommand, config) {
  ...
  const flags = providerCommandFlags(provider, harness, config);
...
module.exports = {
  ...
  launchSpecForTicket, providerChoices, launchSpecForChoices,
};
