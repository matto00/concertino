...
// display titles from cache.js's own tickets.json exactly as the live queue
...
// same as a cold tickets.json is "needs fetching". Every caller (watch.js's
...
function read(root) {
  ...
  return {
    ...
    // CON-69: same degrade-don't-fail stance as inFlight — a malformed
    // per-row spec map costs only the overrides (those tickets fall back to
    // the batch command on restore), never the queue itself. Entries are
    // kept only when they look exactly like what queue.js baked in: a
    // string command, and env either absent/null or a flat object.
    perTicket: (parsed.perTicket && typeof parsed.perTicket === 'object' && !Array.isArray(parsed.perTicket))
      ? Object.fromEntries(Object.entries(parsed.perTicket).filter(([, v]) =>
        v && typeof v.command === 'string' &&
...
function write(root, queue, sessionId, now) {
  const payload = {
    ...
    // CON-69: the per-row spec map ({ id: { command, env } }) rides along so
    // a restored queue's overridden tickets still launch under their chosen
    // harness/speed/provider — null when the batch is uniform, exactly as
    // in-memory (see queue.js's createQueue).
    perTicket: (queue && queue.perTicket) || null,
  };
