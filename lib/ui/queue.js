...
function createQueue(tickets, maxConcurrent, launchCommand, confirmed, perTicket) {
  return {
    ...
    // CON-69: identifier -> { command, env } for tickets whose per-row
    // launch-plan choices (or labels, pre-resolved at confirm time) differ
    // from the batch command — the caller spawns these verbatim instead of
    // re-deriving from launchCommand. null (never {}) when the whole batch
    // is uniform, so pre-CON-69 records and uniform queues stay
    // shape-identical. Carried through every queue rebuild below and
    // persisted by queue-cache.js, same as launchCommand.
    perTicket: perTicket || null,
  };
}
...
function tick(queue, runs) {
  ...
  return {
    ...
    queue: Object.assign(
      { pending, inFlight, maxConcurrent: queue.maxConcurrent, launchCommand: queue.launchCommand, confirmed: true, perTicket: queue.perTicket || null },
...
function createRestoredQueue(record, runs, reconciled) {
  ...
  return {
    ...
    perTicket: record.perTicket || null,
    confirmed: false,
    ...
    queue: Object.assign(
      {
        ...
        perTicket: queue.perTicket || null,
      },
      queue.restoredFrom ? { restoredFrom: queue.restoredFrom } : {},
    ),
  };
