...
//   1. The persisted snapshot under `<root>/.concertino/runs/<TICKET_ID>/
//      evidence/`, written by persist-evidence.sh — the more honest source
//      when it exists (it is exactly what the run worked from), and the only
//      one that survives cleanup.sh --phase4 destroying the worktree.
...
// Depth-first search under `dir` for a file literally named `filename`,
// returning its absolute path or null. Bounded to a handful of files (an
// evidence directory holds only this run's own planning/report artifacts),
// so a full walk on every drilldown poll is cheap.
//
// Why search rather than reconstruct the path: fix-persist-evidence-path-
// collision (CON-23) changed persist-evidence.sh to preserve `ticket.md`'s
// path relative to its own worktree top-level (e.g.
// `openspec/changes/<change>/ticket.md` for this project's openspec spec
// provider, but a project configured with a different specProvider.changeDir
// — or none at all — nests it differently). Reconstructing that exact shape
// here would require this dashboard module to read and interpret
// concertino.config.json's specProvider convention, which it does not do
// anywhere else (lib/ui is otherwise driven purely from `.concertino/`
// runtime state, never the project config) — hardcoding one project's
// convention (`openspec/changes/...`) would silently break for any other.
// Searching finds `ticket.md` wherever persist-evidence.sh actually put it,
// regardless of spec provider, and needs no additional plumbing. Safe
// because `ticket.md` is written exactly once per run during Planning
// (orchestrator.md Phase 1) — there is never more than one candidate.
function findFile(dir, filename) {
  let entries;
  try {
    entries = fs.readdirSync(dir, { withFileTypes: true });
  } catch (e) {
    return null; // missing/unreadable evidence dir — nothing to find
  }
  // Files before subdirectories: a same-level match (the common case — most
  // runs persist ticket.md at their evidence dir's own top level, or one
  // level down) resolves without descending further.
...
function persistedPath(root, ticket) {
  const evidenceDir = path.join(root, '.concertino', 'runs', ticket, 'evidence');
  return findFile(evidenceDir, 'ticket.md');
}
...
function resolve(root, ticket, cache) {
  let content = null;
  const filePath = persistedPath(root, ticket);
  if (filePath != null) {
    try {
