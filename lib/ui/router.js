...
const sessions = require('./screens/sessions');
const presets = require('./screens/presets');
const archive = require('./screens/archive');
const compare = require('./screens/compare');
...
const SCREENS = {
  ...
  // The sessions screen (CON-78) — mode = 'sessions' is entered ONLY from
  // the fleet screen's `v` key (fleet/keys.js's handleKey -> 'open-sessions');
  // see screens/sessions.js's own header comment and design.md Decision 8.
  sessions: { render: sessions.render, handleKey: sessions.routeHandleKey },
  // The PRESETS screen (CON-111) — mode = 'presets' is entered ONLY from the
  // settings screen's `p` key (settings.js's handleKey -> 'open-presets');
  // see screens/presets.js's own header comment and design.md Decision 6.
  presets: { render: presets.render, handleKey: presets.routeHandleKey },
  // The run-archive screen (CON-113) — mode = 'archive' is entered ONLY
  // from the fleet screen's `A` key (fleet/keys.js's handleKey ->
  // 'open-archive'); see screens/archive.js's own header comment and
  // design.md Decision 1.
  archive: { render: archive.render, handleKey: archive.routeHandleKey },
  // The side-by-side run-comparison screen (CON-114) — mode = 'compare' is
  // entered ONLY via 'open-compare' (archive.js's `c` binding in its list
  // zone, or fleet/keys.js's `c` binding on a DONE row), once exactly two
  // DONE runs are marked; see screens/compare.js's own header comment and
  // design.md Decision 2.
  compare: { render: compare.render, handleKey: compare.routeHandleKey },
};
