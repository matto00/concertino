...
// Packs footer key hints ('↵ attach', 'q quit', ...) into as many dim lines
// as `cols` requires — two-space leading indent, three-space separators, the
// same shape every screen's single `f.dim('  ' + hints.join('   '))` footer
// already renders — so a hint list wider than the terminal WRAPS instead of
// being truncated (every frame line is eventually clamped to cols, by the
// screen's own final map or buildFrame's padTo, so an over-wide single line
// silently loses its tail: on an 80-col terminal the fleet footer lost
// everything from `n new run` onward whenever a queue was active). A hint
// never breaks internally — items are the wrap unit — and a single item
// wider than cols still gets its own line (the downstream clamp handles it,
// as before). cols <= 0 (unknown width) degrades to the old single line.
function hintLines(items, cols) {
  const all = (items || []).filter(Boolean);
  if (!all.length) return [];
  if (!(cols > 0)) return [dim('  ' + all.join('   '))];
  const lines = [];
  let cur = '';
  for (const item of all) {
    const candidate = cur ? cur + '   ' + item : '  ' + item;
    if (cur && visibleLength(candidate) > cols) {
      lines.push(cur);
      cur = '  ' + item;
    } else {
      cur = candidate;
...
// Multi-row sibling of sparkline() above — additive, independent, and
// non-disruptive to it (sparkline() is left as its own implementation, not
// reimplemented in terms of this function, so its call sites are untouched).
// Stacks `rows` block-character rows to reach a combined `rows *
// SPARK_LEVELS.length` sub-level resolution instead of sparkline()'s fixed
// 8, for callers with enough vertical room to spend on chart resolution
// (see fleet/metrics.js's expanded-tier throughput chart, and the
// multi-row-metrics-charts design doc's Decisions 1/2).
//
// Returns an array of exactly `rows` strings, each `values.length` long,
// ordered TOP ROW FIRST (index 0 is the topmost row, matching print order —
// index `rows - 1` is the bottommost row). Normalization mirrors
// sparkline()'s own "divide by the series max" approach, just against the
// combined resolution: each value maps onto a `totalLevel` in
// `[0, rows * SPARK_LEVELS.length - 1]`, then each row band (0 = bottom,
// `rows - 1` = top) renders blank space for a band entirely above that
// column's height, the fully-filled glyph for a band entirely below it, and
// the partial-fill glyph for the band the height falls within. A `0` value
// against a non-zero max still renders `SPARK_LEVELS[0]` (not blank) at the
// bottom row, matching sparkline()'s existing zero-value convention there —
// only bands strictly above a bar's current height are ever blank.
function multiRowSparkline(values, rows) {
  const arr = values || [];
  const max = arr.reduce((m, v) => Math.max(m, v || 0), 0);
  const totalLevels = rows * SPARK_LEVELS.length;
  const levels = arr.map((v) => (
    max === 0 ? 0 : Math.round((Math.max(0, v || 0) / max) * (totalLevels - 1))
  ));
  const out = [];
  for (let b = rows - 1; b >= 0; b--) {
    const bandFloor = b * SPARK_LEVELS.length;
    let row = '';
    for (const totalLevel of levels) {
      const subLevel = totalLevel - bandFloor;
      if (subLevel < 0) row += ' ';
      else if (subLevel >= SPARK_LEVELS.length - 1) row += SPARK_LEVELS[SPARK_LEVELS.length - 1];
      else row += SPARK_LEVELS[subLevel];
    }
    out.push(row);
  }
  return out;
...
module.exports = {
  dur, truncate, padTo, bar, sparkline, multiRowSparkline, visibleLength, stripUnsafeControls, hintLines,
