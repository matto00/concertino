...
// Writes a `run.spawn` event the instant the dashboard itself creates a
// ticket's tmux window (CON-77) — the spawn is a fact the dashboard already
// owns, so it records it in-process rather than waiting for the launched
// agent to report in via setup-worktree.sh's run.start. Matches
// emit-event.sh's own wire shape (t/kind/project/ticket/role) field-for-field
// (see design.md Decision 2) so reducer.js/store.js need no special-casing
// for which process wrote a given line. Deliberately minimal — no
// harness/provider guess; run.start remains the authoritative source for
// those once the agent gets that far.
//
// Never throws: a lost run.spawn event must never block or fail a real
// spawn, the same "telemetry must never fail a delivery run" contract
// emit-event.sh upholds structurally.
function writeSpawnEvent(root, ticket) {
  try {
    ...
    // Swallow — see header comment above.
  }
}

// CON-98, design.md Decision 2: writes a `run.override` event — the manual,
// dashboard-only bucket override behind the `d` ("mark done") confirm gate.
// Mirrors writeSpawnEvent above field-for-field (t/kind/project/ticket/role,
// plus `status`), for the same reason: a TUI action recording a fact about
// itself, in-process, without going through emit-event.sh. Never throws —
// same "telemetry must never fail the dashboard action" contract.
function writeOverrideEvent(root, ticket, status) {
  try {
    const runDir = path.join(root, '.concertino', 'runs', ticket);
    fs.mkdirSync(runDir, { recursive: true });
    const line = JSON.stringify({
    ...
    // Swallow — see writeSpawnEvent's header comment above.
  }
}
...
function createSession(name, root) {
  const target = (ticket) => {
  ...
  return {
    ...
    // `env` (optional, CON-65): a { NAME: value } map injected into the
    // window's command as an `env NAME='value' ...` prefix. A prefix, not
    // `tmux new-window -e`: the real command reaches the pane via
    // respawn-window (see below), which has no `-e` of its own — and a
    // prefix keeps the injected environment visible in the window's own
    // command line (`tmux list-windows -F '#{pane_start_command}'`), which
    // is worth having when debugging why one run routed differently from
    // its neighbours. Values are single-quoted via shQuote (./shquote.js —
    // the standard '"'"' escape for embedded quotes); names are validated
    // against the POSIX-portable class rather than quoted, since a name
    // that needs quoting is a bug in the caller, not a case to support.
    spawn(ticket, cmd, env) {
      if (env && Object.keys(env).length) {
        const parts = [];
        for (const [key, value] of Object.entries(env)) {
          if (!/^[A-Za-z_][A-Za-z0-9_]*$/.test(key)) {
            throw new Error('refusing to inject unsafe env name: ' + JSON.stringify(key));
          }
          parts.push(key + '=' + shQuote(value));
        }
        cmd = 'env ' + parts.join(' ') + ' ' + cmd;
      }
      ...
      // CON-77: record the spawn itself as telemetry, the instant the window
      // that will run `cmd` actually exists — mirroring where run.start
      // semantically belongs relative to "the thing actually happened".
      // Only when a root was supplied (see createSession's own comment);
      // omitting it is a no-op, unchanged from before this feature existed.
      if (root) {
        writeSpawnEvent(root, ticket);
      }
    },
...
// CON-78: the sessions view's own generalised attach/kill — a
// `session:window_id` pair for ANY tmux session on the machine, not just
// this project's own (createSession()'s returned object only ever addresses
// windows inside ITS session, by ticket name). window_id (`@N`), not window
// name: tmux window ids are unique machine-wide, so this needs no
// `assertAddressable`-style ticket validation the way `target()` above does
// — the caller (the sessions controller) already resolved this pair from a
// live `tmux list-panes` row, not from unvalidated user input.
//
// Mirrors attach()'s/kill()'s own shape exactly (design.md Decision 7 /
// tasks.md 2.1-2.2) — a freelance tmux-backed session gets the same
// suspend-terminal attach and the same swallow-everything kill a
// Concertino-managed one already has, just addressed by an arbitrary
// session name instead of this module's own `name`.
function attachTarget(sessionName, windowId) {
  return spawnSync('tmux', ['attach', '-t', sessionName + ':' + windowId], { stdio: 'inherit' });
}

function killTarget(sessionName, windowId) {
  try { tmux(['kill-window', '-t', sessionName + ':' + windowId]); } catch (e) {}
}

module.exports = {
  hasTmux, createSession, PLACEHOLDER, attachTarget, killTarget, writeOverrideEvent,
};
