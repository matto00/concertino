...
//
// CON-20 (skeptic-final-1.md): bumped again for `teamFound` — a cache row
// written before this field existed cannot possibly know whether its
// `teamKey` resolved, and reading it as `teamFound: null` (falling out of
// the schema-mismatch -> empty() path below, exactly like every prior bump)
// is the correct default: `null` means "unknown", never mistaken for
// `false` (which would paint a stale, pre-this-change row as an error it
// never actually was) and never for `true` either (see openLaunchPad's own
// comment in watch.js for why `null` — not `true` — is what a fresh process
// needs here).
...
function read(root) {
  ...
  return {
    ...
    // A cache written before `truncated` existed was, in fact, not truncated
    // by MAX_TICKETS (the concept didn't exist yet) — defaulting a missing
    // value to `false` here is simply true, not a masked absence, so no
    // schema version bump is needed (design.md Decision 4).
    truncated: typeof parsed.truncated === 'boolean' ? parsed.truncated : false,
    // CON-20: whether `teamKey` above actually resolved against Linear when
    // this row was written — `true` (resolved, real team, however many
    // tickets), `false` (refreshLaunchPad's resolveTeam confirmed no team
    // matches this key), or `null` (unknown — a row this schema version
    // guarantees was written WITH this field, so `null` here means the
    // writer legitimately didn't know, not that it's missing/stale; see
    // watch.js's refreshLaunchPad, the only writer). openLaunchPad in
    // watch.js reads this to rebuild the team-not-found error on a fresh
    // process, without waiting for a new `r` refresh.
    teamFound: typeof parsed.teamFound === 'boolean' ? parsed.teamFound : null,
  };
}
...
function write(root, data, now) {
  const payload = {
    ...
    // CON-20: `false` must round-trip as `false`, never fall back to `null`
    // via `||` — that's exactly the "real value collapsed into absence" bug
    // linear.js's own `priority` comment already treats as a wall, and
    // `teamFound: false` is the one value this whole field exists to carry.
    teamFound: typeof (data && data.teamFound) === 'boolean' ? data.teamFound : null,
  };
...
module.exports = { CACHE_SCHEMA_VERSION, cacheDir, cachePath, empty, read, write, age, isCold, clear };
