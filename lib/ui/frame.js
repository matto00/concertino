...
// CON-112: SGR mouse-reporting mode (button-event tracking, `?1000`, plus
// extended SGR coordinates, `?1006`, so clicks past column/row 223 still
// report correctly). Named and paired exactly like ALT_SCREEN_ENTER/
// ALT_SCREEN_EXIT/CURSOR_HIDE/CURSOR_SHOW above — one enable, one matching
// disable, written from watch.js at the same points raw mode itself is
// toggled (design.md Decision 1), so this pairing is grep-verifiable the
// same way. `?1006` alone would report legacy X10 coordinates from `?1000`,
// which parseMouseClick's regex below does not match — see design.md's
// "Risks" note; that failure mode is a silent no-op, not a crash.
const MOUSE_REPORT_ENTER = '\x1b[?1000h\x1b[?1006h';
const MOUSE_REPORT_EXIT = '\x1b[?1000l\x1b[?1006l';
...
// CON-112, design.md Decision 2: recognizes a left-button-press SGR mouse
// sequence (`\x1b[<0;Cx;CyM` — button code `0` = left button, final byte `M`
// = press; a release is the same shape with a final `m`, deliberately NOT
// matched here since only press is handled this pass) and returns its
// 1-based terminal `{ row, col }`, or `null` for anything else (a release,
// any other button/motion event, or an ordinary CSI sequence — all of which
// fall through to the existing keypress path unchanged). Pure, and given a
// single already-split `key` token — no change to `splitKeys` itself is
// needed: an SGR sequence already ends in `M`/`m`, both within splitKeys'
// own CSI-final-byte range (`@`-`~`), so it already arrives here as one
// complete token, exactly like an arrow key does.
const MOUSE_CLICK_RE = /^\x1b\[<0;(\d+);(\d+)M$/;
function parseMouseClick(key) {
  const m = MOUSE_CLICK_RE.exec(key);
  if (!m) return null;
