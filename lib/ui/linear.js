...
// The team key is the prefix of every ticket id — CON-1 is team CON. Taking it
// from the id example keeps it out of the config surface entirely.
function teamKeyFromConfig(config, env) {
  const e = env || process.env;
  if (e.LINEAR_TEAM_KEY) return e.LINEAR_TEAM_KEY;
  const example = ((config || {}).ticketProvider || {}).idExample || '';
  const m = /^([A-Za-z][A-Za-z0-9]*)-\d+$/.exec(example.trim());
