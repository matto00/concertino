...
// CON-62: `concertino validate --ticket <ID>` needs one issue's `labels`
// (to check for a `harness:<value>` override), not the full launch-pad
// shape — a separate, much smaller query rather than reusing QUERY's
// team-scoped connection for a single, already-known id.
const ISSUE_QUERY = `
query ConcertinoOneTicket($id: String!) {
  issue(id: $id) {
    id
    identifier
    labels(first: 20) { nodes { name } }
...
// CON-62 (design.md Decision 6, tasks.md 4.2): a minimal single-issue fetch
// for `concertino validate --ticket <ID>` — reuses the same transport/auth
// plumbing (`postRaw`, `LINEAR_API_KEY` from env) as every other call in
// this file rather than a second client. `id` accepts a human-facing
// identifier (e.g. "CON-62"), same as Linear's `issue(id:)` field does.
async function fetchOneTicket({ apiKey, id, transport } = {}) {
  const t = transport || httpsTransport;
  ...
  const data = await postRaw(t, key, ISSUE_QUERY, { id });
  if (!data || !data.issue) {
    throw new Error('linear: ticket "' + id + '" was not found');
  }
...
function normaliseTicket(node) {
  ...
  return {
    ...
    // CON-192 (followup-survival-report): additive-only — needed so survival
    // can be computed without a second lookup. `null` when Linear omits the
    // field (an in-progress/open ticket never has completedAt).
    createdAt: toMillis(node.createdAt),
    completedAt: toMillis(node.completedAt),
