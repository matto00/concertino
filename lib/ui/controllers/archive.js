...
// also imported by controllers/fleet.js, so the cap/DONE-only rules live in
// exactly one place.
...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    // tasks.md 1.2/1.3: re-resolves the target run fresh from S.runs at
    // handling time (never trusts a value cached from keypress time — same
    // discipline drilldown.js's 'move-drill-evidence' etc. already follow)
    // so the shared helper's DONE-only/cap rules see the CURRENT status,
    // not a stale one. Also handled, identically, by controllers/fleet.js —
    // see that module's own matching comment for why this is intentional
    // duplication of the switch-case wiring, not the underlying logic
    ...
    // tasks.md 2.4: opens the compare screen, recording that it was opened
    // FROM archive (S.mode is still 'archive' here — S.compareReturnMode is
    // read back by controllers/compare.js's 'back-to-origin-from-compare').
    // Re-checks the 2-selected precondition (archive.js's own handleKey
    // already gates on it) — the same belt-and-braces discipline
    // drilldown.js's kill/restart confirmations apply, since `state` here
    // could in principle have drifted between the keypress and this
    // handler running. A stale/short selection is a no-op, not a crash.
    case 'open-compare': {
