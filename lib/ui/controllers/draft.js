...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    // the launch pad's own provider gate (ticket-provider.js's
    // launchPadStatus) — the draft flow is Linear-only for v1, and a
    // non-Linear provider is told so inline rather than the prompt silently
    // doing nothing.
    case 'open-ticket-draft': {
      // CON-93 item 3: `ctx.deps.linear` is ticket-provider.js's resolver
      // (see watch.js's own header comment), so kindFor resolves the
      // deprecated `manual` alias to `local` here exactly as
      // local.js's own launchPadStatus gate already benefits from —
      // comparing the raw, unaliased ctx.config.ticketProvider.kind would
      // give a `manual`-configured project the wrong ("this project uses
      // manual") message below.
      const kind = ctx.deps.linear.kindFor(ctx.config || {});
      if (kind !== 'linear') {
        if (S.prompt) {
          S.prompt.error = kind === 'local'
