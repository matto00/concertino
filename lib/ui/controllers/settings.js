...
// CON-111: the PRESETS screen's own openPresets() — called DIRECTLY here
// (not through the applyAction loop) so 'open-presets' stays this
// controller's own case while the actual session-building logic stays owned
// by controllers/presets.js, mirroring the split every other cross-screen
// entry point in this codebase already uses (e.g. controllers/launchpad.js's
// ensureLaunchPad, called from watch.js rather than duplicated).
...
function openSettings(ctx) {
  ...
  S.settings = {
    ...
    focus: 'sections',
    prompt: null,
    // CON-72: the enum-list multi-select's own sub-state, null unless one is
    // open — same lifecycle as `prompt` beside it.
    chooser: null,
    saveError: null,
  };
  S.mode = 'settings';
...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    // CON-111: delegates to controllers/presets.js's own openPresets() —
    // kept as a case HERE (rather than registered under the 'presets'
    // controller's own action-type set) because the settings screen's `p`
    // key is the only entry point into mode = 'presets', exactly like
    // 'open-settings' above is the fleet screen's own entry point into this
    // controller.
    case 'open-presets':
      presetsController.openPresets(ctx);
      return true;
    ...
    case 'settings-open-field-prompt': {
      ...
      if (!entry) return true;
      const { value } = settingsScreen.currentValue(S.settings.candidate, entry);
      // CON-72: a scalar-list seeds as its own comma-separated rendering, so
      // what the prompt opens with is exactly what committing it back
      // unchanged would produce — no lossy round trip.
      const seeded = entry.kind === 'scalar-list'
        ? (Array.isArray(value) ? value.join(', ') : '')
        : (value != null ? String(value) : '');
      S.settings.prompt = { path: action.path, value: seeded, error: null };
      return true;
    }

    // --- CON-72: the enum-list multi-select --------------------------------

    // Opens the chooser over the schema's declared values, pre-ticked from
    // the candidate's current membership. `selected` is an ARRAY, not a Set:
    // order is meaningful for `harnesses` (it decides the rendered default
    // harness), so a toggle appends rather than reordering what is already
    // chosen.
    case 'settings-open-chooser': {
      if (!S.settings) return true;
      const entry = S.settings.fieldMeta.get(action.path);
      if (!entry || !entry.items || !entry.items.enum) return true;
      const { value } = settingsScreen.currentValue(S.settings.candidate, entry);
      const current = Array.isArray(value) ? value.slice() : [];
      S.settings.chooser = {
        path: action.path,
        options: entry.items.enum.slice(),
        selected: current.filter((v) => entry.items.enum.includes(v)),
        index: 0,
      };
      S.settings.saveError = null;
      return true;
    ...
    case 'settings-chooser-move': {
      if (!S.settings || !S.settings.chooser) return true;
      const ch = S.settings.chooser;
      ch.index = Math.max(0, Math.min(ch.index + action.delta, ch.options.length - 1));
      return true;
    }

    case 'settings-chooser-toggle': {
      if (!S.settings || !S.settings.chooser) return true;
      const ch = S.settings.chooser;
      const opt = ch.options[ch.index];
      if (opt == null) return true;
      const at = ch.selected.indexOf(opt);
      if (at === -1) ch.selected.push(opt);
      else ch.selected.splice(at, 1);
      return true;
    }

    // Commits membership in the SCHEMA's own declared order rather than
    // toggle order, so the written list is stable across sessions and two
    // people who tick the same boxes get byte-identical config.
    case 'settings-commit-chooser': {
      if (!S.settings || !S.settings.chooser) return true;
      const ch = S.settings.chooser;
      const ordered = ch.options.filter((o) => ch.selected.includes(o));
      configLib.deepSet(S.settings.candidate, ch.path, ordered);
      S.settings.chooser = null;
      S.settings.saveError = null;
      return true;
    }

    case 'settings-cancel-chooser':
      if (S.settings) S.settings.chooser = null;
      return true;
    ...
    case 'settings-commit-prompt': {
      ...
      if (entry && entry.kind === 'scalar-list') {
        // CON-72: split on commas, drop empties (so a trailing comma or an
        // all-whitespace entry never becomes a member), and coerce per the
        // ELEMENT type — `ui.breakpoints` is an integer list, and writing
        // "1024" as a string there would fail validation on save for a
        // reason the typist could not see.
        const numeric = entry.items && (entry.items.type === 'integer' || entry.items.type === 'number');
        value = String(typed).split(',').map((s) => s.trim()).filter(Boolean)
          .map((s) => (numeric ? configLib.coerce(s) : s));
      } else if (entry && (entry.type === 'integer' || entry.type === 'number')) {
        value = configLib.coerce(typed);
      } else {
        value = typed;
      }
