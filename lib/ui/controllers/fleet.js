...
// CON-107, design.md Decision 4: 'open-historical-escalation''s STILL-LIVE
// branch dispatches through the exact same 'open-escalation' handling g/↵
// already use elsewhere — required directly (a peer controller module, safe
// to require the same way settings.js already requires presets.js directly)
// rather than duplicating its S.mode/S.escalationTicket assignment here.
const escalationCtl = require('./escalation');
...
function applyJumpAction(ctx, jump) {
  ...
  switch (jump.type) {
    ...
    case 'focus-quickstart':
      ...
      return true;
    // CON-107, design.md Decision 2: jumping INTO METRICS never touches
    // `selected`/`scrollOffset` — mirrors 'focus-quickstart' immediately
    // above exactly, only for the METRICS-local escalation-history cursor.
    case 'focus-metrics':
      S.multiSelect.failed = new Set();
      if (S.focus === 'queue') S.multiSelect.queued = new Set();
      S.focus = 'metrics';
      S.metricsEscalationFocus = jump.index;
      return true;
    default:
      return false;
  }
...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    // --- CON-107: METRICS' recent-escalations history (design.md) ---------

    // CON-107, design.md Decision 2: jumping INTO METRICS never touches
    // `selected`/`scrollOffset` — mirrors 'focus-quickstart' above exactly.
    case 'focus-metrics':
      return applyJumpAction(ctx, action);

    // j/k while focus is 'metrics' — moves ONLY metricsEscalationFocus,
    // clamped to the CURRENT full escalation history (re-derived fresh here,
    // same "never trust a value from a previous draw()" discipline
    // 'move-quickstart-focus' above applies). Never touches
    // selected/scrollOffset.
    case 'move-metrics-focus': {
      const len = ctx.metricsEscalationHistory().length;
      if (!len) return true; // draw()'s own re-clamp keeps the cursor at 0
      const cur = S.metricsEscalationFocus == null ? 0 : S.metricsEscalationFocus;
      S.metricsEscalationFocus = Math.max(0, Math.min(cur + action.delta, len - 1));
      return true;
    }

    // Escape while focus is 'metrics' — returns to the ordinary run
    // selection WITHOUT hiding the panel (METRICS, like QUICK START, is
    // always visible — there is no toggle to hide it). Mirrors
    // 'exit-quickstart-focus' above.
    case 'exit-metrics-focus':
      S.focus = 'runs';
      return true;

    // `↵` on the METRICS-focused history row (design.md Decision 2/4).
    // `action.index` arrives UNRESOLVED (fleet/keys.js's handleKey has no
    // access to the live history array — see its own header comment), so
    // this handler is the one place that actually resolves it: re-derive
    // the history list fresh (never a value cached from a previous draw())
    // and no-op (no state change) if `action.index` does not resolve to a
    // real entry in THAT list.
    case 'open-historical-escalation': {
      const history = ctx.metricsEscalationHistory();
      const entry = history[action.index];
      if (!entry) return true; // nothing resolved — stale/empty/shrunk list
      if (!entry.resolved) {
        // STILL LIVE (design.md Decision 4): dispatch through the exact
        // same 'open-escalation' handling g/↵ already use elsewhere —
        // never a second, divergent code path (ticket AC 2).
        return escalationCtl.handle({ type: 'open-escalation', ticket: entry.ticket }, ctx);
      }
      // RESOLVED: a read-only historical view — never reuses
      // S.escalationTicket for this (design.md Decision 4's round-2
      // correction — S.escalationHistoryItem is its own, separate field).
      S.mode = 'escalation';
      S.escalationHistoryItem = entry;
      return true;
    }
