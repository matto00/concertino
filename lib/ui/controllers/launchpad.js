...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    case 'cycle-harness': {
      ...
      // (claude-code with an incomplete gateway cannot go local — CON-75
      // otherwise both the direct and gateway routes work) — drop it rather
      // than carry a choice this harness cannot honour, then rebuild the
      // command and the preview against whatever survived.
