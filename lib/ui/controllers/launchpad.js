...
function handle(action, ctx) {
  ...
  switch (action.type) {
    ...
    case 'cycle-harness': {
      ...
      // the codex label.
      // CON-65: provider routing is per-harness too — a project can route
      // codex to Ollama while leaving claude-code on subscription, so the
      // header's provider row must follow the cycle for the same reason the
      // models preview does.
      {
        const ollama = ctx.config && ctx.config.providers && ctx.config.providers.ollama;
        plan.providerDefault = (ollama && (ollama.harnesses || []).includes(harnessCmd.canonicalHarness(plan.harness)))
          ? 'ollama' : 'default';
      }
      // An explicit batch provider may not be reachable from the NEW harness
      // (claude-code without a gateway cannot go local) — drop it rather than
      // carry a choice this harness cannot honour, then rebuild the command
      // and the preview against whatever survived.
      if (plan.provider && !harnessCmd.providerChoices(ctx.config, plan.harness).includes(plan.provider)) {
        plan.provider = null;
      }
    ...
    // that validity, so claude-code without a gateway offers nothing and
    // this is a no-op). Re-applies the provider's launch flags and
    ...
    case 'cycle-provider': {
      ...
      plan.provider = choices[(idx + 1) % choices.length];
    ...
    // label path's own refusals (claude-code needs a gateway; 'default'
    // only means something on a project-routed harness).
