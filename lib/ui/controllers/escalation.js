...
function answerEscalationSub(ctx, ticket, index, value, total) {
  const S = ctx.S;
  // CON-179: look up this sub-question's own text from the live run state
  // (the same escalation.raised-derived `subQuestions` 'open-escalation'
  // above already reads) so it rides alongside the value in answer.json —
  // never trust the action's own `index` alone as "which question this is".
  const run = (S.runs || []).find((r) => r.ticket === ticket);
  const subQuestions = run && run.escalation && run.escalation.subQuestions;
  const question = (Array.isArray(subQuestions) && subQuestions[index]) ? subQuestions[index].question : undefined;
  // CON-179 (cycle-2 finding 4): pass the full subQuestions array too, so a
  // provenance-mismatched file can be discarded-and-restarted rather than
  // permanently refusing every slot as "already answered".
  const result = ctx.deps.store.writeSubAnswer(ctx.root, ticket, index, value, total, question, subQuestions);
  // CON-179 (cycle-3 review, finding 3): a discard must not be silent -- set
  // the notice before branching on ok/complete below so it survives either
  // outcome (advancing a step, or completing and returning to the fleet).
  if (result.ok && result.discarded) {
    S.escalationNotice = 'a previous answer for this escalation did not match and was discarded ('
      + result.discardedCount + ' prior answer' + (result.discardedCount === 1 ? '' : 's')
      + ' lost) — starting over from this answer';
  }
