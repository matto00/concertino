...
// liveEscalations, historicalItem) — exported so watch.js's draw() and this
// module's own tests agree on the SAME boolean rather than keeping two
// independent copies of the same condition in sync by hand.
//
// CON-107, selectable-escalations-list design.md Decision 4 (skeptic round
// 1/2 corrections): `historicalItem` (`S.escalationHistoryItem`) is `null`
// for a live escalation opened via 'open-escalation' — every existing caller
// with only 3 args keeps working unchanged (`!!undefined` is `false`). A
// historical (resolved/timed-out) view carries no `escalationTicket` at all
// (Decision 5 never sets it), so the ORIGINAL condition below is always
// false for it — without this extra branch, the banner for some OTHER run's
// live escalation would incorrectly render on top of a historical detail
// view.
function suppressedOnOwnScreen(mode, escalationTicket, liveEscalations, historicalItem) {
  if (mode === 'escalation' && !!historicalItem) return true;
