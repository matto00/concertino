...
const { isTerminalRunEnd } = require('./run-terminal');
...
function applyEvent(run, ev) {
  ...
  switch (ev.kind) {
    ...
    case 'run.start':
      run.startedAt = ev.t;
      // CON-182 (cold-review cycle 2): a run.start emitted AFTER a prior
      // terminal run.end means this ticket's events.jsonl — append-only,
      // shared across the ticket's whole history — was genuinely re-run.
      // Clear the stale endedAt/endStatus so this run reports live again
      // instead of permanently "done"/"failed" from its FIRST delivery. A
      // no-op for the ordinary case (a run's own first, only run.start,
      // with no prior run.end to clear). See run-terminal.js for the
      // shared "terminal" definition this mirrors from the `run.end` case
      // below, and core/scripts/lib/run-end-status.sh for the identical
      // rule on the bash side (watchdog.sh/cleanup.sh).
      run.endedAt = null;
      run.endStatus = null;
    ...
    case 'run.end':
      // CON-182 (cold-review cycle 2): fix at fold time, not in
      // deriveStatus. A run.end whose status is "escalated" marks an
      // orchestrator pausing on a circuit-breaker escalation, not the run
      // ending — the same run typically resumes once it's answered,
      // sometimes without ever writing a new run.start. NOT recording
      // endedAt/endStatus for it at all (rather than recording it and
      // special-casing 'escalated' in deriveStatus below) is what also
      // fixes lib/ui/reap.js's selectReapable(), which reads
      // `run.endStatus == null` directly and never went through
      // deriveStatus — a live, resumed orchestrator was being captured and
      // killed as "idle past its grace period" 10 minutes after the
      // escalated run.end, exactly the incident this ticket exists to
      // close. It also fixes the fleet/drill-down UI rendering a resumed
      // run as a frozen "escalated · <stale elapsed>" row (fleet/rows.js,
      // drilldown.js both read endStatus/elapsedMs straight off this
      // object). See run-terminal.js's isTerminalRunEnd() for the shared
      // definition (also used by lib/ui/retention.js) and
      // core/scripts/lib/run-end-status.sh for the identical rule in bash.
      if (!isTerminalRunEnd(ev)) break;
      run.endedAt = ev.t;
    ...
    case 'escalation.raised': {
      ...
      run.escalation = {
        ...
        raisedAt: ev.t,
        // CON-188 task 5.3: additive/optional — absent (null) on any
        // pre-CON-188 raise, exactly like context/contextRef above. Cleared
        // on the next escalation.raised (a NEW escalation, same as every
        // other field here) and on resolution (the `run.escalation = null`
        // case just below — unchanged).
        escalationId: ev.escalation_id != null ? ev.escalation_id : null,
        ...
        // CON-156 (cycle 2, finding 2): cleared implicitly whenever a fresh
        // `escalation.raised` rebuilds this object (a NEW escalation, e.g.
        // after this one resolves and another is later raised, starts with
        // no malformed history of its own) — never carried over from a prior
        // `run.escalation`.
    ...
    // CON-156 (cycle 2, finding 2): a malformed multi-part answer.json is
    // reported but NEVER treated as a resolution — deliberately absent from
    // the escalation.answered/escalation.timeout case just below. The
    // escalation stays exactly as open (and exactly as NEEDS YOU) as it was
    // before this event; only the advisory `malformedReason` shown on the
    // escalation screen changes. If `run.escalation` is already gone (a
    // late-arriving event for an escalation that has since genuinely
    // resolved some other way), there is nothing to attach it to — dropped,
    // same as any other event arriving for a state that no longer exists.
    case 'escalation.malformed':
      if (run.escalation) run.escalation.malformedReason = ev.reason || 'malformed answer file';
      break;
...
function deriveStatus(run) {
  ...
  // CON-182: `run.endStatus` is only ever set by applyEvent's `run.end` case
  // for a TERMINAL run.end (see run-terminal.js's isTerminalRunEnd()) — an
  // "escalated" run.end never reaches this field at all (fixed at fold
  // time, not here), so no special-case is needed in this function: a set
  // `endStatus` always really is terminal.
  if (run.endStatus) {
