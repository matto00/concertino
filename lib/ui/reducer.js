...
function deriveStatus(run) {
  ...
  // CON-182: `endStatus === 'escalated'` is NOT terminal — it marks an
  // orchestrator pausing on a circuit-breaker escalation, not the run ending.
  // The same run typically resumes once it's answered, sometimes without
  // ever writing a new run.start (observed on helio HEL-1080, 2026-09-11),
  // so treating it like any other endStatus would report a live, resuming
  // run as FAILED the moment its window looked dead or its escalation object
  // was cleared by `escalation.answered` above. As of this ticket,
  // orchestrator.md no longer emits run.end for an escalation pause, but
  // historic logs still contain such lines — skip the endStatus branch
  // entirely for them and fall through to the same window/escalation/
  // BLOCKER checks used when no run.end has been observed at all. See the
  // matching rule in cleanup.sh's run_end_is_terminal(), watchdog.sh's
  // lane_is_complete(), and lib/ui/retention.js's hasRunEnd().
  if (run.endStatus && run.endStatus !== 'escalated') {
