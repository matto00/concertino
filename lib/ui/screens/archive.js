...
// CON-114, design.md Decision 1: `markedForCompare` renders the shared
// run-comparison selection's own marker (`✓`) — a distinct column from the
// ordinary cursor marker (`▸`/` `) just before it, so a row can show both
// at once (the cursor and the comparison marking are independent facts,
// mirroring fleet/rows.js's own multiSelected-vs-cursor-marker treatment).
function archiveRow(run, isSelected, width, markedForCompare) {
  const marker = isSelected ? '▸' : ' ';
  const compareMark = markedForCompare ? '✓' : ' ';
  ...
  const line = ' ' + marker + compareMark + ' ' + idCol + ' ' + titleCol + ' ' + harnessCol + ' ' + statusCol + ' ' + startedCol;
...
function renderArchive(state, opts) {
  ...
  // CON-114, design.md Decision 1: the shared run-comparison selection —
  // marked runs render the `✓` indicator regardless of which screen most
  // recently toggled them (spec.md's "A run marked in fleet's DONE section
  // shows as marked in archive too" scenario).
  const compareSelection = s.compareSelection || [];

  if (filtered.length) {
    out.push(archiveHeaderRow(cols));
    for (let i = 0; i < filtered.length; i++) {
      const marked = compareSelection.indexOf(filtered[i].ticket) !== -1;
      out.push(archiveRow(filtered[i], focus === 'list' && i === selected, cols, marked));
    }
  } else {
  ...
  if (focus === 'query') hints.push('type to filter');
    ...
    hints.push('j/k move', '↵ open drilldown', 'space mark for compare');
...
function handleKey(key, state) {
  ...
  if (focus === 'list') {
    ...
    // CON-114, design.md Decision 1 / tasks.md 2.1: `space` marks the run
    // under the cursor for comparison, dispatched unconditionally for
    // whatever's under the cursor (any status) — the shared controller-side
    // helper (lib/ui/compare-selection.js), not this screen, is the single
    // place the DONE-only rule is enforced (tasks.md 1.3's "cap/DONE-only
    ...
    if (key === ' ') {
      ...
      const run = filtered[selected];
      if (!run) return null;
      return { type: 'toggle-compare-select', ticket: run.ticket };
    }
    // tasks.md 2.3 / design.md Decision 4: opens the compare screen once
    // exactly two runs are marked; a no-op otherwise (mirrors this screen's
    // own existing precedent of silently ignoring an action whose
    // precondition isn't met, e.g. `↵` on an empty filtered list).
    if (key === 'c') {
      const selection = s.compareSelection || [];
      if (selection.length === 2) return { type: 'open-compare' };
      return null;
    }
    return null;
  }
