...
// The box's own top/bottom border rows — docview.bodyBox() always adds these
// ON TOP of whatever content/viewportRows it renders, exactly like
// docview.js's own BOX_BORDER_ROWS. Lazygit-layout pass (CON-43): CHROME_ROWS
// _BASE above never accounted for these two rows — `computeViewportRows`
// under-reserved by exactly 2, a latent bug that pre-dates this change but
// was masked before it: `bodyBox` previously only reached
// `viewportRows + BOX_BORDER_ROWS` rows when content actually needed
// windowing (rare — most tickets fit in one screen), so the 2-row overflow
// was never exercised by the ticket's own live repro (a short description).
// Now that `bodyBox` grows to fill `viewportRows` on every finite-`rows`
// render (design.md Decision 4), the same miscount would overflow on every
// single render, not just the windowed case — confirmed via probe (a 60-line
// description at rows: 20 already produced 21 lines against a 19-line budget
// BEFORE this change too — `git stash` + re-run reproduced the identical
// off-by-2 on the pre-change source). Included here, matching docview.js's
// own `DOC_CHROME_ROWS = 4 + BOX_BORDER_ROWS` pattern exactly.
...
function computeViewportRows(rows, hasUrl) {
  ...
  const chrome = CHROME_ROWS_BASE + BOX_BORDER_ROWS + (hasUrl ? 1 : 0);
...
function renderTicketView(ticket, opts) {
  ...
  if (!ticket) {
    return [sectionHeader({ icon: icons.ticket, label: 'TICKET', colour: f.bold }),
      '', f.dim('  ticket no longer in the cache'), '', f.dim('  esc back')].join('\n');
  }

  const out = [];
  out.push(sectionHeader({
    icon: icons.ticket,
    label: f.bold(ticket.identifier || '?') + '  ' + f.truncate(ticket.title || '', cols - 12),
  }));
