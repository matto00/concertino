...
// icons.js has a fitting glyph only for the `description` field — no
// existing glyph fits `title` or `acceptanceCriteria` (dashboard-iconography's
// restricted vocabulary — see icons.js's own header comment on why a new
// glyph is not invented ad hoc for either). Those two fields' headers are
// rendered via `sectionHeader` with no `icon`, which returns the label
// unchanged (design.md Decision 4) — no visible change for them.
const FIELD_ICONS = { description: icons.description };
...
function renderTicketDraft(draft, opts) {
  ...
  for (const def of FIELDS) {
    ...
    const marker = active ? '▸ ' : '  ';
    boxContent.push(marker + sectionHeader({
      icon: FIELD_ICONS[def.name],
