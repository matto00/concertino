...
// indicator. Only content that exceeds `viewportRows` is windowed down to
// fit it.
//
// Lazygit-layout pass (design.md Decision 4): when `viewportRows` is finite
// (a positive `rows` was supplied by the caller — `renderDocView`'s
// `computeViewportRows`/ticketview.js's own equivalent already resolve an
// absent/`0` `rows` to `Infinity`), the box height GROWS to fill that
// viewport (`Math.max(content.length, viewportRows) + BOX_BORDER_ROWS`)
// instead of always sizing to the (possibly shorter) natural content height —
// so a document shorter than the viewport still fills it, matching the
// established `fleet.js`/`drilldown.js`/`launchpad.js` grow-to-fill pattern.
// `content` is already windowed/capped to `viewportRows` above, so `Math.max`
// only ever grows, never shrinks, the already-windowed case. When
// `viewportRows` is `Infinity` (unbounded), height stays `content.length +
// BOX_BORDER_ROWS` exactly as before this change — `bodyBox` is called by
// exactly two places (`renderDocView`, `ticketview.js`), both full-screen
// compositions that want this; an embedded, non-growing caller does not
// exist today but would need its own opt-out if one is ever added (design.md
// risk log).
function bodyBox(bodyLines, opts) {
  ...
  const height = Number.isFinite(viewportRows)
    ? Math.max(content.length, viewportRows) + BOX_BORDER_ROWS
    : content.length + BOX_BORDER_ROWS;
...
function renderDocView(doc, opts) {
  ...
  // No icon here — this module's exports stay generic/reusable and know
  // nothing about any caller's own concept (see this file's own header
  // comment); a caller that wants an icon prefix composes it into `title`
  // itself before this ever renders. `sectionHeader` still owns the
  // colour-wrap, so this is the one place every icon+label (or plain-label)
  // composition goes through, per dashboard-iconography's widened
  // requirement.
  const out = [];
  out.push(sectionHeader({ label: f.truncate(title, cols), colour: f.bold }));
  out.push('');
