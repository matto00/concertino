...
function ticketRow(n, ticket, plan, index) {
  ...
  // A ticket whose `harness:<value>` label re-dispatches it to a different
  // CLI than the batch's (plan.ticketHarness, computed by watch.js at plan
  // creation — CLI-label space on both sides, so `===` is the whole
  // comparison). Rendered only when it actually differs from the CURRENT
  // batch harness: cycling 'h' onto the same harness makes the marker
  // disappear because at that point the label changes nothing. The marker
  // lives INSIDE the title column's fixed 30-col budget (title yields the
  // space) so annotated rows keep the same ports/label column alignment as
  // every other row and nothing is pushed past the truncate-to-cols edge.
  // CON-69: an explicit per-row choice (plan.rowOverrides, cycled with
  // H/S/P on the focused row) wins over the ticket's own label-derived
  // values (plan.ticketHarness/plan.ticketProvider, CON-62/CON-65) — the
  // same precedence confirm-launch applies when it bakes the batch into the
  // queue record, so the marker never promises a spawn the confirm would
  // not actually produce.
  const ro = (plan.rowOverrides && plan.rowOverrides[ticket.identifier]) || {};
  const labelHarness = plan.ticketHarness && plan.ticketHarness[ticket.identifier];
  const effectiveHarness = ro.harness || labelHarness;
  // CON-65: a `provider:<value>` label's routing rides the same marker —
  // `⇒ ollama` alone, `⇒ codex·ollama` combined with a harness override.
  // plan.ticketProvider only ever contains tickets whose label genuinely
  // re-routes the spawn (resolveTicketProvider's own null cases filter the
  // rest), so unlike the harness half there is no batch value to compare
  // against — presence is the whole signal.
  const effectiveProvider = ro.provider || (plan.ticketProvider && plan.ticketProvider[ticket.identifier]);
  const noteParts = [];
  if (effectiveHarness && effectiveHarness !== plan.harness) noteParts.push(effectiveHarness);
  if (ro.speed && ro.speed !== plan.speed) noteParts.push(ro.speed);
  if (effectiveProvider) noteParts.push(effectiveProvider);
  const note = noteParts.length ? '⇒ ' + noteParts.join('·') : '';
  const titleField = note
    ? f.padTo(f.truncate(ticket.title || '', 30 - note.length - 3), 30 - note.length - 1) + ' ' + f.cyan(note)
    : f.padTo(name, 30);
  // CON-69: the row cursor. '▸' (cyan) marks the row H/S/P act on — plain
  // three-space gutter everywhere else so unfocused rows are byte-identical
  // to the pre-cursor rendering.
  const cursor = plan.rowIndex === index ? ' ' + f.cyan('▸') + ' ' : '   ';
  return cursor + n + '  ' + f.padTo(ticket.identifier, 9) + ' ' + titleField +
