...
function overrideNote(ticket, plan) {
  ...
  if (effectiveProvider) parts.push('via:' + providerWord(effectiveProvider));
...
function renderLaunchPlan(plan, activeCount, opts) {
  ...
  out.push(cfgRow('provider', providerWord(plan.providerDefault === 'ollama' ? 'ollama' : 'default'),
    plan.providerConfigured
      ? (perRow ? 'P per-row' : 'pinned by dashboard.launchCommand')
      : 'set `providers.ollama` in config to use P'));
