...
function ticketRow(n, ticket, plan, index) {
  ...
  const override = plan.ticketHarness && plan.ticketHarness[ticket.identifier];
  ...
  const providerOverride = plan.ticketProvider && plan.ticketProvider[ticket.identifier];
  const noteParts = [];
  if (override && override !== plan.harness) noteParts.push(override);
  if (providerOverride) noteParts.push(providerOverride);
  const note = noteParts.length ? '⇒ ' + noteParts.join('·') : '';
  const titleField = note
    ? f.padTo(f.truncate(ticket.title || '', 30 - note.length - 3), 30 - note.length - 1) + ' ' + f.cyan(note)
    : f.padTo(name, 30);
  return '   ' + n + '  ' + f.padTo(ticket.identifier, 9) + ' ' + titleField +
