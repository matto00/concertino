...
function renderSettings(settings, opts) {
  ...
  const leftContent = sections.length
    ...
    : emptyState({ message: '  (no sections)' });
  ...
  const rightContent = fields.length
    ...
    : emptyState({ message: '  (no fields in this section)' });

  const paneHeight = Math.max(leftContent.length, rightContent.length) + 2;
  // No icons.js glyph fits a schema section name generically (each of
  // `project`/`ui`/`dashboard`/... is config-specific, not a fixed concept
  // this restricted vocabulary already covers) — routed through
  // sectionHeader with no icon (task 4.10: escalate rather than invent a new
  // glyph ad hoc; omitting the icon is the documented fallback, design.md
  // Decision 4).
  const sectionsTitle = sectionHeader({ label: 'SECTIONS' });
  const fieldsTitle = sectionHeader({ label: f.truncate((currentSection || '(none)').toUpperCase(), rightContentW) });
  ...
  if (settings.chooser) {
    ...
    for (const line of emptyState({ message: '(no field selected)' })) detailContent.push(line);
  }
  ...
  if (settings.chooser) {
    ...
    hints.push('S save', 'p presets', 'esc discard');
...
function handleKey(key, state) {
  ...
  // CON-111: lowercase `p` — deliberately distinct from Capital S above and
  // from every field-navigation key below — opens the PRESETS screen (create/
  // rename/delete/edit named launch presets). Bound at this top level (no
  // prompt/chooser open — both already returned above), so it works from
  // either pane, matching every other top-level entry point on this screen.
  if (key === 'p') return { type: 'open-presets' };
