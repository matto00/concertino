...
// CON-107, design.md Decision 4: the read-only branch for a RESOLVED (or
// timed-out) METRICS history entry — reached only via `renderEscalation`'s
// own `opts.historical` precedence check above. Reuses the exact same box/
// textwrap/pane machinery the live render below uses (`pane`, `sectionHeader`,
// `textwrap.wrap`) rather than a second, divergent module — never binds an
// option key, shows the recorded `decision`/"no answer recorded" in place of
// answer controls, and its footer is `esc back` only.
function renderHistoricalEscalation(entry, opts) {
  const cols = Math.max(40, opts.cols || 80);
  const now = opts.now != null ? opts.now : Date.now();

  const out = [];
  out.push(f.bold('concertino') + f.dim(' · ' + entry.ticket));
  out.push('');

  const tag = f.dim('[history]');

  // CON-46-style wizard detection, reused as-is: a historical multi-part
  // entry is never re-entered step by step — design.md Decision 1 already
  // flattened its answers into one `decision` string — so the box shows
  // exactly the LAST/most-recently-answered sub-question (never
  // subQuestions[0]) alongside a note that more steps exist.
  const wizard = isWizard(entry);
  const currentStep = wizard ? entry.subQuestions[entry.subQuestions.length - 1] : null;
  const currentQuestion = wizard ? (currentStep.question || '') : entry.question;
  const currentOptions = wizard ? (currentStep.options || []) : (entry.options || []);

  const boxWidth = cols;
  const innerWidth = Math.max(0, boxWidth - BOX_BORDER_PADDING_COLS);
  const boxContent = [];
  boxContent.push(sectionHeader({ label: tag }));
  if (wizard && entry.subQuestions.length > 1) {
    boxContent.push(f.dim(entry.subQuestions.length + ' sub-questions, see decision below'));
  }
  boxContent.push('');
  for (const line of textwrap.wrap(currentQuestion, innerWidth)) boxContent.push(line);
  boxContent.push('');

  // Full option list, no key bindings — this escalation is already resolved,
  // there is nothing left to answer here (design.md Decision 4).
  for (const opt of currentOptions) boxContent.push('  ' + opt);
  if (currentOptions.length) boxContent.push('');

  boxContent.push(entry.timedOut
    ? f.dim('no answer recorded (timed out)')
    : (entry.decision != null ? 'decision: ' + entry.decision : f.dim('no answer recorded')));

  const meta = [];
  if (entry.role) meta.push('raised by ' + entry.role);
  if (entry.resolvedAt != null) {
    const verb = entry.timedOut ? 'timed out' : 'answered';
    meta.push(verb + ' ' + f.dur(Math.max(0, now - entry.resolvedAt)) + ' ago');
  }

  // Lazygit-layout pass grow-to-fill, mirrored from the live render below —
  // `belowBoxRows` tracks every push that follows the box, in lockstep.
  ...
  const budget = opts.rows > 0 ? opts.rows - 1 : 0;
...
function renderEscalation(run, opts) {
  ...
  const historical = (opts && opts.historical) || null;

  // design.md Decision 4: a historical entry takes precedence over EVERY
  // live-run check below, checked FIRST — it needs no live `run` at all
  // (render()'s router wrapper safely passes `run: null` in this case, the
  // same shape a vanished run already renders through the `!run` branch just
  // below, just with this check ahead of it).
  if (historical) {
    return renderHistoricalEscalation(historical, { cols, now, rows: opts && opts.rows });
  }
  ...
  // CON-156 (cycle 2, finding 2): a malformed answer.json's diagnostic
  // (blank separator + one warning line), rendered further below —
  // independent of `notice` (a transient write-refusal message) since a
  // malformed-file warning persists across polls, not just one screen visit.
  if (esc.malformedReason) belowBoxRows += 2;
  belowBoxRows += 1; // blank before the footer
  belowBoxRows += 1; // footer line
  ...
  // CON-156 (cycle 2, finding 2): surfaces the same diagnostic emit-event.sh
  // wrote to `escalation.malformed` for — a human/agent staring at this
  // screen must see the same thing a --wait-only poller's stderr would show,
  // not just infer from the wait never ending that something is wrong.
  if (esc.malformedReason) {
    out.push('');
    out.push('  ' + f.red(f.truncate('answer.json is malformed: ' + esc.malformedReason, cols - 4)));
  }
...
function render(state, opts) {
  ...
  return renderEscalation(run, Object.assign({}, opts, {
    ...
    // CON-107, design.md Decision 5: threaded through alongside the existing
    // escalationTicket lookup above.
    historical: state.escalationHistoryItem || null,
  }));
}
...
function routeHandleKey(key, state) {
  ...
  return handleKey(key, {
    run, reply: state.escalationReply, subIndex: state.escalationSubIndex,
    historical: state.escalationHistoryItem || null,
  });
}
