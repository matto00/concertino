...
// Lazygit-layout pass: the drill-down's own panel order — index 0-4 maps to
// digit keys 1-5 and is what \t cycles through. Distinct from PHASE_ORDER
// above; this is a fixed 5-entry list, never derived from run data. Replaces
...
// CON-104: 'changes' (the CHANGES panel) is the fifth entry, added alongside
// the original four — see changesLines()/changesItems() below.
const DRILL_PANELS = ['ticket', 'timeline', 'gates', 'evidence', 'changes'];
...
const RIGHT_MIN = 20;
// CON-104 (design.md Decision 6): raised from 34 — `git diff --stat` lines
// are typically `<path> | <N> +++---`, which routinely exceeds the old cap
// for a nested path. `leftContentW`'s own floor (minLeftContent) and the
// `cols - totalOverhead - minLeftContent` cap on the right column (see
// renderDrillDown, below) still protect TIMELINE from being crushed by this.
const RIGHT_MAX = 50;
const NO_GATES_WIDTH = 'no gate results recorded'.length;
const NO_EVIDENCE_WIDTH = 'no evidence recorded'.length;
// CHANGES' own "nothing to show" fallback is short — unlike the worktree-
// removed message (WORKTREE_REMOVED_MSG), which is long enough that sizing
// the column to fit it unconditionally would crush TIMELINE for no good
// reason; that message is allowed to truncate at a narrow terminal, same as
// a pathologically long gate first_error already is (see the loop below).
const NO_CHANGES_WIDTH = 'no changes'.length;

function rightContentWidth(run, diffStat) {
  const gates = run.gates || [];
  ...
  // EVIDENCE/CHANGES share this column, and their own "nothing recorded"
  // fallbacks are the common case today — never size so tight that either
  // fallback message itself would need truncating.
  w = Math.max(w, NO_EVIDENCE_WIDTH);
  w = Math.max(w, NO_CHANGES_WIDTH);
  // CON-104: the diff-stat lines' own widths (design.md Decision 6) — the
  // same "size to content, capped" treatment gate lines already get above.
  for (const file of changesItems(diffStat)) {
    // selection-marker prefix (2 cols) + the stat line itself.
    w = Math.max(w, 2 + f.visibleLength(file.line));
  }
...
// --- CON-104: CHANGES panel — git diff --stat against the run's worktree --
//
// `diffStat` is `S.drillDiffStat` (watch.js's draw(), gated to the ticket
// currently drilled into — see design.md Decision 1/2): `null` is the
// sentinel for "no worktree to diff" — either the worktree was never set,
// or `fs.existsSync` failed this poll (unset and removed-by-cleanup both
// render the same honest "worktree removed" message; this panel is not in
// a position to distinguish them). Otherwise `{ stat, error }` — `stat` is
// an array of `{ path, line }` on success (empty means a real diff with
// zero changes), `error` is a message when the `git` call itself failed or
// timed out this poll.

// The diff-stat entries a `diffStat` value implies — shared by
// changesLines() below and by watch.js (to clamp `drillChangesIndex`'s
// bounds) and drilldown.js's own handleKey (to resolve the selected file's
// path when `↵` opens it — see design.md Decision 3).
function changesItems(diffStat) {
  return (diffStat && Array.isArray(diffStat.stat)) ? diffStat.stat : [];
}

// Same fixed-cap-with-a-scrolling-window shape as EVIDENCE's own
// EVIDENCE_MAX_VISIBLE/evidenceWindow (design.md Decision 3) — a busy
// worktree with many changed files should not grow the CHANGES box
// unboundedly any more than a busy EVIDENCE list does.
const CHANGES_MAX_VISIBLE = 8;

function changesWindow(total, selectedIndex, focused) {
  if (!focused) {
    return { start: 0, count: Math.min(total, CHANGES_MAX_VISIBLE) };
  }
  const win = layout.selectionWindow(total, selectedIndex, CHANGES_MAX_VISIBLE, selectedIndex);
  return { start: win.start, count: win.count };
}

// The fixed degradation string for "no worktree to diff" (design.md
// Decision 2) — exported so watch.js/tests can match against the exact same
// string rather than a loosely-worded duplicate.
const WORKTREE_REMOVED_MSG = 'worktree removed — CHANGES is only available while a run\'s worktree exists';

function changesLines(diffStat, width, opts) {
  const o = opts || {};
  const focused = !!o.focused;
  const selectedIndex = Math.max(0, o.selectedIndex || 0);

  // No worktree to diff at all — covers both "never set" and "removed since
  // this drill-down was opened" (design.md Decision 2); no git call is even
  // attempted for either (see watch.js's draw()).
  if (!diffStat) {
    return [f.yellow(WORKTREE_REMOVED_MSG)];
  }
  // The `git` call itself failed or timed out this poll — a transient
  // condition (a mid-write worktree, a slow disk) that self-heals on the
  // next poll tick, never a thrown error or a silently empty panel.
  if (diffStat.error != null || diffStat.stat == null) {
    return [f.yellow('diff unavailable')];
  }
  const files = diffStat.stat;
  if (!files.length) {
    return [f.yellow('no changes')];
  }

  const { start, count } = changesWindow(files.length, selectedIndex, focused);
  const lines = [];
  for (let i = start; i < start + count; i++) {
    const file = files[i];
    const isSelected = focused && i === selectedIndex;
    const prefix = isSelected ? '▸ ' : '  ';
    const text = prefix + file.line;
    lines.push(f.truncate(isSelected ? f.bold(text) : text, width));
  }
  const hidden = files.length - (start + count);
  if (!focused && hidden > 0) {
    lines.push(f.dim('  … ' + hidden + ' more'));
  }
  return lines;
...
// track-per-run-cost-spend, specs/drilldown-run-cost/spec.md: the run's
// accumulated cost — `run.costUsd`/`run.tokens` are set by reducer.js's
// run.cost fold (null until at least one run.cost event has landed). Three
// distinct states, never collapsed into one (design.md Decision 4 of the
// parent change): a real figure; an explicit non-Claude-Code "not reported
// for <harness>" notice (mirroring the exact wording style
// lib/ui/controllers/fleet.js's `address-failure` claude-code-only notice
// already uses); or a generic "not reported" notice for a Claude Code run
// that simply has no data yet (feature disabled at sync time, or the run
// predates this feature) — deliberately worded differently from the
// harness-specific case so the two are never confused for the same reason.
function costText(run) {
  if (run.costUsd != null) {
    const dollar = '$' + run.costUsd.toFixed(2);
    if (!run.tokens) return dollar;
    const totalTokens = Object.values(run.tokens).reduce((sum, n) => sum + (n || 0), 0);
    return dollar + '  ' + totalTokens.toLocaleString() + ' tok';
  }
  if (run.harness && run.harness !== 'claude-code') {
    return f.dim('cost not reported for ' + run.harness);
  }
  return f.dim('cost not reported');
}
...
function headerLines(run, cols, ticketText) {
  ...
  // track-per-run-cost-spend: a fifth row for the run's accumulated cost —
  // same truncate-not-wrap treatment as row4.
  const row5 = f.truncate(costText(run), cols);

  return [row1, titleRow, row2, row3, row4, row5];
}
...
function renderDrillDown(run, opts) {
  ...
  // CON-104: the CHANGES panel's own diff-stat data (S.drillDiffStat,
  // computed per-poll by watch.js's draw() — see changesItems()'s own
  // comment above) and selection index, threaded through exactly like
  // drillEvidenceIndex above.
  const changesFocused = drillFocus === 'changes';
  const drillChangesIndex = Math.max(0, (opts && opts.drillChangesIndex) || 0);
  const diffStat = (opts && opts.diffStat) || null;
  ...
  // GATES/EVIDENCE/CHANGES (per design.md Decision 2; CHANGES added by
  // CON-104's design.md Decision 6) are kept as three separate stacked
  // boxes, not merged into one — so each can independently lose its border
  // under layout.degrade() and so the ticket's own "timeline/gates/evidence
  // panels", plural, are four legible panels, not two. Lazygit-layout pass:
  // all five panels (TICKET/TIMELINE/GATES/EVIDENCE/CHANGES) are now
  // individually focusable (digit keys 1-5, or \t to cycle) and render with
  ...
  const rightContentW = Math.max(RIGHT_MIN,
    Math.min(rightContentWidth(run, diffStat), cols - totalOverhead - minLeftContent));
  ...
  const changesTitle = sectionHeader({ icon: icons.changes, label: '[5] CHANGES' });
  ...
  // CON-104: CHANGES is a third stacked pane below EVIDENCE (design.md
  // Decision 6) — sized the same way GATES/EVIDENCE are, with its own
  // per-poll diffStat/selection state.
  const changesContent = changesLines(diffStat, rightContentW, { focused: changesFocused, selectedIndex: drillChangesIndex });

  const gatesBoxHeight = gatesContent.length + 2;
  const evidenceBoxHeightNatural = evidenceContent.length + 2;
  const changesBoxHeightNatural = changesContent.length + 2;
  const rightTotalHeight = gatesBoxHeight + evidenceBoxHeightNatural + changesBoxHeightNatural;
  const leftNaturalHeight = timelineContent.length + 2;
  // Height reconciliation across hsplit()'s panes is the caller's job
  // (design.md Decision 3): the right column is now three boxes whose
  // combined border overhead (6 rows) exceeds the left column's single box
  // (2 rows), so whichever side is naturally shorter gets padded — via
  // box()'s own `height` option, which blank-pads any content rows beyond
  // what the content itself provides — up to the taller side's total. In the
  // common case (TIMELINE and GATES+EVIDENCE+CHANGES already carry roughly
  // the same amount of raw content), this is exactly "pad TIMELINE's content
  // by extra blank rows to absorb the right column's extra border overhead".
  ...
  if (confirm) {
    ...
    const f1 = footer.footer({ hints: ['1-5 jump', 'tab cycle', 'j/k select', '↵ open', 'esc back'], cols });
    footerLines = f1.lines;
    footerRowCount = f1.rows;
  } else if (changesFocused) {
    // CON-104 (design.md Decision 5/6): a distinct branch from evidenceFocused
    // — not folded into it — since the two panels' hint sets differ: CHANGES'
    // own selection/open hints are shown ONLY while the diff-stat list has at
    // least one file (spec.md's "keys advertised only when they currently do
    // something"), unlike EVIDENCE's own (unconditional-once-focused) hints
    // above. The focus switch itself is never blocked either way — an empty
    // CHANGES panel is still a legitimate `5`/tab target, it simply advertises
    // no selection/open keys while there is nothing to select.
    const hints = ['1-5 jump', 'tab cycle'];
    if (changesItems(diffStat).length > 0) hints.push('j/k select', '↵ open');
    hints.push('esc back');
    const f3 = footer.footer({ hints, cols });
    footerLines = f3.lines;
    footerRowCount = f3.rows;
  } else {
    ...
    // currently holds focus (only EVIDENCE/CHANGES focus, above, block them).
    const hints = ['1-5 jump', 'tab cycle', '↑/↓ scroll', '↵ attach'];
  ...
  // CON-104: CHANGES is now the LAST box in the right column, so it (not
  // EVIDENCE) absorbs the slack that keeps the right column's total in step
  // with `targetHeight` — GATES/EVIDENCE both size to their own natural
  // content, exactly as before this change.
  const changesBoxHeight = changesBoxHeightNatural + Math.max(0, targetHeight - rightTotalHeight);
  ...
  const evidenceBox = pane(evidenceContent,
    { width: rightPaneWidth, height: evidenceBoxHeightNatural, title: evidenceTitle, focused: evidenceFocused });
  const changesBox = pane(changesContent,
    { width: rightPaneWidth, height: changesBoxHeight, title: changesTitle, focused: changesFocused });

  const rightColumn = gatesBox.concat(evidenceBox).concat(changesBox);
  ...
  if (confirm) {
    ...
    // While EVIDENCE or CHANGES holds focus, ↵ attach / k kill / r restart
    // are neither bound (handleKey below) nor advertised — the whole point
    // of the focus gate (design.md Decision 3; spec.md's "Evidence focus
    // exposes selection and open, not attach/kill/restart", extended to
    // CHANGES by this change's own design.md Decision 6). Which hint set
    // applies was decided above, ahead of the height budget — see `footerLines`.
...
// drillEvidenceIndex, drillChangesIndex, diffStat }), returns an action for
// watch.js to carry out, or null for "no-op, no redraw". Never mutates, same
// seam as escalation.js.
function handleKey(key, state) {
  ...
  // CON-104: mirrors drillEvidenceIndex above — `diffStat` is state.diffStat
  // (S.drillDiffStat), the same per-poll value threaded into render()'s opts.
  const drillChangesIndex = Math.max(0, (state && state.drillChangesIndex) || 0);
  const diffStat = (state && state.diffStat) || null;
  ...
  const items = evidenceItems(run);
  const changesFiles = changesItems(diffStat);

  // Lazygit-layout pass: digit 1-5 jumps directly to a panel; \t cycles
  // through all five in DRILL_PANELS order. Both are unconditional — unlike
  // the old EVIDENCE-only toggle, every panel (including an empty EVIDENCE
  // or CHANGES) is a legitimate focus target now, since TICKET/TIMELINE/
  // GATES also scroll their own content.
  ...
  // CON-104: CHANGES mirrors EVIDENCE's own focus/selection pattern exactly
  // (design.md Decision 3) — ↵ attach / k kill / r restart are inert while
  // CHANGES holds focus for the same reason they are for EVIDENCE (j/k are
  // claimed for selection instead). Selection movement is dispatched
  // regardless of whether the list is currently empty (clamped downstream by
  // the controller, mirroring move-drill-evidence) — only the open key (↵)
  // is a no-op with nothing selected.
  if (drillFocus === 'changes') {
    if (key === 'j' || key === '\x1b[B') return { type: 'move-drill-changes', delta: 1 };
    if (key === 'k' || key === '\x1b[A') return { type: 'move-drill-changes', delta: -1 };
    if (key === '\r') {
      const file = changesFiles[drillChangesIndex];
      if (!file) return null;
      // Never shells out itself — the renderer stays pure. watch.js's
      // 'open-diff-doc' controller handler does the actual `git diff` read.
      return { type: 'open-diff-doc', ticket: run.ticket, worktree: run.worktree, file: file.path, label: file.path };
    }
    return null;
  }
  ...
  // this screen cannot give j/k a second meaning the way EVIDENCE/CHANGES
  // do).
...
function render(state, opts) {
  ...
  return renderDrillDown(run, Object.assign({}, opts, {
    ...
    // CON-104: threaded straight off `state` (part of currentState()'s own
    // snapshot — see app-state.js), unlike ticketText above (which arrives
    // via `opts` instead, since it is never needed by routeHandleKey below —
    // CHANGES' own selection logic needs the same diffStat/index pair here
    // AND there, so both live on `state`).
    drillChangesIndex: state.drillChangesIndex,
    diffStat: state.drillDiffStat,
  }));
}
...
function routeHandleKey(key, state) {
  ...
  return handleKey(key, {
    ...
    drillChangesIndex: state.drillChangesIndex, diffStat: state.drillDiffStat,
  });
}
