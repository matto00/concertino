...
function renderDrillDown(run, opts) {
  ...
  for (const line of pane(ticketContent,
    { width: cols, height: ticketContent.length + 2, title: '[1] TICKET', focused: ticketFocused })) out.push(line);
  out.push('');
  ...
  const timelineTitle = '[2] TIMELINE' + (run.malformed
    ? '  ' + f.yellow('▲ ' + run.malformed + ' malformed')
    : '');
  const gatesTitle = '[3] GATES' + (run.cycle != null ? ' · cycle ' + run.cycle : '');
  const evidenceTitle = '[4] EVIDENCE';
  ...
  if (rows > 0) {
    ...
    // blank line after it, and the confirm/notice/footer block — `out`
    // already holds every header row, the phase pipeline, and the TICKET
    // panel by this point in the function.
    const belowRow = 2;
  ...
  if (confirm) {
    ...
    // selection and open, not attach/kill/restart"). j/k are EVIDENCE's own
    // (select, not scroll) — distinct from the arrow-only scroll the other
    // three panels use, so the same letters never mean two things on this
    // screen depending on focus.
