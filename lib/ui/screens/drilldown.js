...
function renderDrillDown(run, opts) {
  ...
  const rightWidth = Math.max(RIGHT_MIN, Math.min(rightContentWidth(run), cols - sepWidth - minLeft));
  const leftWidth = Math.max(minLeft, cols - sepWidth - rightWidth);

  // A crashed emitter mid-write leaves malformed lines that reducer.js
  // already counts per run (run.malformed) — fleet.js sums it fleet-wide
  // but attributes it to no single run. This screen's entire job is
  // explaining ONE run, so its own corrupted-event count belongs here, next
  // to the timeline it undermines: without it, a run with gaps in its own
  // log renders as a clean, merely-short history — absent data reading as
  // healthy data, on the one screen whose job is to say otherwise.
  const timelineHeader = 'TIMELINE' + (run.malformed
    ? '  ' + f.yellow('▲ ' + run.malformed + ' malformed')
    : '');
  const left = [timelineHeader, ...timelineLines(run, leftWidth)];
  const right = [
