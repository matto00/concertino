...
function renderDrillDown(run, opts) {
  ...
  for (const line of pane(ticketContent,
    { width: cols, height: ticketContent.length + 2, title: icons.ticket + ' [1] TICKET', focused: ticketFocused })) out.push(line);
  out.push('');
  ...
  const timelineTitle = icons.timeline + ' [2] TIMELINE' + (run.malformed
    ? '  ' + f.yellow('▲ ' + run.malformed + ' malformed')
    : '');
  const gatesTitle = icons.gates + ' [3] GATES' + (run.cycle != null ? ' · cycle ' + run.cycle : '');
  const evidenceTitle = icons.evidence + ' [4] EVIDENCE';
