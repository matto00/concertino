...
function renderDrillDown(run, opts) {
  ...
  let footerRows;
  if (confirm) {
    footerRows = ['confirm-placeholder', 'confirm-placeholder']; // 2 rows, built inline below
  } else if (evidenceFocused) {
    footerRows = f.hintLines(['1-4 jump', 'tab cycle', 'j/k select', '↵ open', 'esc back'], cols);
  } else {
    ...
    hints.push('esc back');
    footerRows = f.hintLines(hints, cols);
  }
  ...
  if (rows > 0) {
    ...
    const belowRow = 1 + (notice ? 2 : 0) + footerRows.length;
  ...
  if (confirm) {
    ...
    // was decided above, ahead of the height budget — see `footerRows`.
    for (const line of footerRows) out.push(line);
  }
