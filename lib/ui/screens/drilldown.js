...
function evidenceItems(run) {
  return (run.events || []).filter((ev) => ev.kind === 'evidence');
}
...
function evidenceWindow(total, selectedIndex, focused) {
  ...
  let start = selectedIndex >= EVIDENCE_MAX_VISIBLE ? selectedIndex - EVIDENCE_MAX_VISIBLE + 1 : 0;
  start = Math.max(0, Math.min(start, Math.max(0, total - EVIDENCE_MAX_VISIBLE)));
  return { start, count: Math.min(total - start, EVIDENCE_MAX_VISIBLE) };
}
...
function evidenceLines(run, width, opts) {
  ...
  for (let i = start; i < start + count; i++) {
    ...
    const text = (isSelected ? '▸ ' : '  ') + (ev.label || ev.ref || '(untitled)');
...
function renderDrillDown(run, opts) {
  ...
  // \t-gated EVIDENCE focus (design.md Decision 3, mirrors launchpad.js's
  // own epics/tickets pane switch) — `evidenceFocused` decides both the
  // panel's border style and which entry (if any) the selection highlights.
  const evidenceFocused = (opts && opts.drillFocus) === 'evidence';
  ...
  if (confirm) {
    ...
    // selection and open, not attach/kill/restart"). `\t` itself is
    // deliberately left off this hint line, matching launchpad.js's own
    // pane-switch key, which is likewise never advertised in its footer.
    out.push(f.dim('  ↑/↓ select   ↵ open   esc back'));
  } else {
    const hints = ['↵ attach'];
...
function handleKey(key, state) {
  ...
  const drillFocus = state && state.drillFocus;
  ...
  // \t toggles EVIDENCE focus (design.md Decision 3) — a no-op, and never
  // advertised, when there is nothing to select at all: evidenceLines()
  // rendering its "no evidence recorded" fallback means `drillFocus` can
  // never usefully become 'evidence' (spec.md's "Focus switch is inert when
  // there is nothing to select").
  if (key === '\t') {
    if (!items.length) return null;
    return { type: 'switch-drill-focus', focus: drillFocus === 'evidence' ? null : 'evidence' };
  }
