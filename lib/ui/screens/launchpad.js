...
// emphasis: bold in the focused pane, dim in the unfocused one, so the two
...
function epicRow(epic, selected, paneFocused) {
  ...
  return paneFocused ? f.bold(line) : f.dim(line);
}
...
// 'tickets'`) decides bold vs. dim. Before this change the marker itself was
...
function ticketRow(ticket, checked, selected, paneFocused, runs, width) {
  ...
  return paneFocused ? f.bold(truncated) : f.dim(truncated);
}
...
function renderLaunchPad(lp, runs, opts) {
  ...
  const leftBoxOpts = { width: leftPaneWidth, height: paneHeight, title: 'EPICS', focused: epicsFocused };
  const rightBoxOpts = { width: rightPaneWidth, height: paneHeight, title: ticketsTitle, focused: ticketsFocused };

  const leftBox = layout.degrade(leftPaneWidth, paneHeight)
    ? ['EPICS', ...leftContent].map((l) => f.truncate(l, leftPaneWidth))
  ...
  // hints line that always follow this pane; one more row is reserved for
  // the trailing newline the writer appends (fleet.js's identical `rows - 1`
  // convention), so a bounded render never scrolls the header off the top.
  ...
  if (!(rows > 0) || availableForDetail >= layout.MIN_BOX_HEIGHT) {
    const detailHeight = rows > 0 ? Math.min(desiredDetailHeight, availableForDetail) : desiredDetailHeight;
