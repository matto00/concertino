...
//
// `queueState` (CON-41, design.md Decision 1) is checked SECOND, after the
// live-run check and before the Linear-state fallback: a ticket present in
// `queueState.pending` or `queueState.inFlight` but with no live run yet
// renders `⏳ queued`, distinct from `▲ running`. `inFlight` is checked here
// (not just `pending`) as belt-and-braces for the brief window between
// `queue.tick()` admitting a ticket and `runs` actually reflecting its live
// run — see design.md Decision 1 for why the alternative (pending-only) was
// rejected. A live run always wins so an actively-running ticket never
// regresses to `queued` on a render that races ahead of `runs`.
function inlineStatus(ticket, runs, queueState) {
  const run = (runs || []).find((r) => r.ticket === ticket.identifier);
  if (run && run.status !== 'done' && run.status !== 'failed') return '▲ running';
  if (queueState && (
    (queueState.pending && queueState.pending.includes(ticket.identifier)) ||
    (queueState.inFlight && queueState.inFlight.has(ticket.identifier))
  )) return '⏳ queued';
...
//
// `queueState` (CON-41) extends the same refusal to an already-queued ticket
// — admitting it again would be a silent duplicate-queue hazard of the same
// shape. `queueState` is an optional third parameter: `undefined` behaves as
// "no queue" (inlineStatus never returns `⏳ queued` without one), preserving
// every pre-existing two-arg call site's behaviour unchanged (design.md
// Decision 4).
function isSelectable(ticket, runs, queueState) {
  const status = inlineStatus(ticket, runs, queueState);
...
function selectableIdentifiers(tickets, runs, queueState) {
  return (tickets || []).filter((t) => isSelectable(t, runs, queueState)).map((t) => t.identifier);
}
...
function headerLine(lp, cols, now) {
  ...
  // CON-20 design.md Decision 4: a zero-ticket fetch against a team Linear
  // confirms is real is success, not an error — say so by name instead of a
  // bare "0 open" that reads identically to a misconfigured team key. Gated
  // on `!lp.error` because a zero-ticket fetch WITH an error is exactly the
  // "team does not resolve" case (watch.js's refreshLaunchPad), which the
  // f.red(lp.error) line below already explains — this header must not
  // contradict that by claiming the team is real. The team name is read from
  // `lp.cache.teamKey` (what was actually queried), never re-derived, so the
  // header can never disagree with the fetch that produced it.
  const teamKey = lp.cache && lp.cache.teamKey;
  const openLabel = total === 0 && !lp.error && teamKey
    ? 'no open tickets in ' + teamKey
    : total + ' open';
  const right = openLabel + truncatedMarker + ' · ' + staleness + ' · r refresh';
...
function ticketRow(ticket, checked, selected, paneFocused, runs, queueState, width) {
  const marker = selected ? '▸' : ' ';
  const box = checked ? '[x]' : '[ ]';
  const status = inlineStatus(ticket, runs, queueState);
  const statusCol = status === '▲ running' ? f.STATUS_COLOUR.running(status)
    : status === '⏳ queued' ? f.STATUS_COLOUR.queued(status)
    : status;
...
function renderLaunchPad(lp, runs, opts) {
  ...
  //
  // CON-20: deliberately `lp.cache.fetchedAt == null`, NOT
  // `cache.isCold(lp.cache)`. `isCold` treats "zero tickets" and "never
  // fetched" identically BY DESIGN (cache.js's own isCold comment) — right
  // for deciding whether the launch pad is USABLE, but wrong here: a real
  // fetch that confirmed the team exists and legitimately has zero open
  // tickets ALSO has `tickets.length === 0`, so `isCold` alone cannot tell
  // "never fetched" apart from "fetched, confirmed-empty" — and taking this
  // branch for the confirmed-empty case rendered "no tickets cached yet —
  // press r to fetch" directly under a header that already says `fetched 0s
  // ago`, an on-screen contradiction (evaluator Change Request 1). Checking
  // `fetchedAt` directly (matching every genuinely-cold-cache test's own
  // fixture — none of them set `fetchedAt`) fixes that: a confirmed-empty
  // team falls through to the normal render below instead, where the
  // header's own "no open tickets in CON" is the only thing said about it —
  // no separate body copy needed, since the header already carries the
  // message the evaluator asked for.
  //
  // `!lp.error`: a fetch that came back empty BECAUSE the team key doesn't
  // resolve (watch.js's refreshLaunchPad) also leaves the cache at zero
  // tickets, and that case has an lp.error that must reach the f.red(...)
  // line below, not be swallowed by this "press r to fetch" hint — which is
  // exactly the silent-failure this whole change exists to fix. A cold
  // cache with NO error still hits this branch exactly as before.
  if ((!lp.cache || lp.cache.fetchedAt == null) && !lp.refreshing && !lp.error) {
  ...
  if (lp.refreshing) {
    out.push('  ' + f.dim('fetching tickets…'));
  }
  if (lp.error) {
  ...
  for (let i = ticketsStart; i < Math.min(tickets.length, ticketsStart + MAX_TICKETS_VISIBLE); i++) {
    const t = tickets[i];
    rightContent.push(ticketRow(t, lp.selected.has(t.identifier), i === lp.ticketIndex, ticketsFocused, runs, queueState, rightContentW));
  ...
  // The footer hints, computed HERE — ahead of the height budget below — so
  // f.hintLines' wrapped row count is actually reserved rather than pushing
  // the frame past `rows`. The hint list itself is unchanged apart from the
  // wrapping (see fleet.js's own hints comment for why wrapping replaced
  // the old single joined line: every frame line is clamped to cols
  // downstream, so an over-wide line silently lost its trailing hints).
  ...
  // Advertised only when the highlighted ticket is actually eligible (same
  // "only hint a key that currently does something" discipline as the
  // clear-queue hint just above) — reads the identical
  // isSelectable(currentTicket(lp), runs, queueState) check the 'q'
  // keypress handler itself gates on, so the two can never disagree.
  if (selectedTicket && isSelectable(selectedTicket, runs, queueState)) hints.push('q add to queue');
  hints.push('esc back');
  ...
  // hint row(s) that always follow this pane (clearQueueConfirm replaces
  // the hint rows with its own two-line warning — see below); one more row
  // is reserved for the trailing newline the writer appends (fleet.js's
  // identical `rows - 1` convention), so a bounded render never scrolls the
  // header off the top.
  const rows = (opts && opts.rows) || 0;
  const reservedBelow = clearQueueConfirm ? 4 : 2 + hintRows.length;
  ...
  if (clearQueueConfirm) {
    ...
    // Built (and budgeted for) above, ahead of the detail pane's height
    // math — see the `hints`/`hintRows` block by `reservedBelow`.
...
function handleKey(key, state) {
  ...
  // Mirrors renderLaunchPad's identical gate above (CON-20: `fetchedAt ==
  // null`, not `cache.isCold`, so a confirmed-empty team isn't locked to
  // "r-only" the way a genuinely never-fetched cache is — its epics/tickets
  // panes are legitimately empty but otherwise-normal keys like esc/switch-
  // pane still apply; `!lp.error` keeps a team-not-found error out of this
  // r-only lockdown too, the rest of the screen behaves normally there).
  if ((!lp.cache || lp.cache.fetchedAt == null) && !lp.refreshing && !lp.error) {
  ...
  if (lp.pane === 'tickets') {
    ...
    // 'q' — add only the highlighted ticket to the queue (CON-41,
    // design.md Decision 2). Unbound elsewhere in this screen's own
    // handleKey and not intercepted globally (fleet.js's 'q' quit binding
    // is only consulted while mode === 'fleet' — see design.md Decision 2
    // for why reusing the letter across screens is safe here). Eligibility
    // (not already running or queued) is re-checked at the actual mutation
    // site in watch.js, not here — this only describes the keypress.
    if (key === 'q') return { type: 'add-to-queue' };
  }
