...
function renderLaunchPad(lp, runs, opts) {
  ...
  if (clearQueueConfirm) {
    ...
    // (design.md's own Risks/Trade-offs note). Placed after L/C, ahead of
    // esc back only: on a narrow terminal that must truncate the hints line,
    // this is the first hint to go, preserving the higher-precedent L/C/esc
    // hints every pre-CON-41 fixture already relies on being visible.
