...
// CON-188 task 5.1/5.2: pairs a raise to its resolution by `escalation_id`
// when both carry one — an identity lookup, not the positional guess this
// replaces. `byId` never removes an entry once closed (unlike the old
// `open` pointer), because a timeout followed by a late answer on the SAME
// escalation_id (the chat-after-timeout path, ticket.md AC3) must be able to
// update the same entry: the append-only log records both events, and this
// walk applies them in order, so the LAST event for a given id wins — which
// is always the answer when one exists, since a timeout can never occur
// after an answer already resolved the escalation.
//
// The `open` pointer is kept ONLY for the id-less fallback (pre-CON-188
// events, or a straddling upgrade — design.md Risks): it tracks the
// most-recently-raised entry that itself carries no escalation_id, exactly
// reproducing the old event-order pairing for exactly the events that have
// no id to pair by. An id-bearing resolution whose id matches no known raise
// is ignored outright (proposal.md's "ignore a resolution whose id matches
// no raise") — it never falls back to the positional pointer, which would
// risk misattributing it to an unrelated escalation.
function buildEscalationHistory(list) {
  const history = [];
  for (const r of list) {
    let open = null; // id-less fallback: most-recently-opened, still-open, id-less entry
    const byId = new Map();
    for (const ev of r.events || []) {
      if (ev.kind === 'escalation.raised') {
        const entry = {
          ...
          raisedAt: ev.t, escalationId: ev.escalation_id != null ? ev.escalation_id : null,
          resolved: false, decision: null, resolvedAt: null, timedOut: false, resolutionChannel: null,
        };
        history.push(entry);
        if (entry.escalationId) {
          byId.set(entry.escalationId, entry);
        } else {
          open = entry;
        }
      } else if (ev.kind === 'escalation.answered') {
        const target = ev.escalation_id ? byId.get(ev.escalation_id) || null : open;
        if (!target) continue; // orphaned resolution — never surfaced
        target.resolved = true;
        target.resolvedAt = ev.t;
        target.timedOut = false;
        target.resolutionChannel = ev.resolution_channel != null ? ev.resolution_channel : null;
        if (Array.isArray(target.subQuestions) && target.subQuestions.length && ev.sub_answers != null) {
          const subAnswers = parseSubAnswers(ev.sub_answers);
          target.decision = target.subQuestions
            .map((sq, i) => (sq.question || '') + ': ' + (subAnswers[i] == null ? '' : subAnswers[i]))
            .join('; ');
        } else {
          target.decision = ev.answer != null ? ev.answer : null;
        }
