  ...
  sections.forEach((s, i) => {
    ...
    for (let k = w.startOffset; k < w.startOffset + w.shown; k++) {
      ...
      if (s.kind === 'queued') {
        ...
        for (const line of renderFinishedRow(s.group[k], { cols: innerCols, avgDoneMs, searchQuery, multiSelected }, rowIndex === selected)) { contentLines.push(line); contentLineOwners.push(rowIndex); }
      } else {
        const rowIndex = sectionStartIndex + k;
