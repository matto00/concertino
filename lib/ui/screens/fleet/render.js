    ...
    if (gridWin.columnAreaHeight >= GRID_MIN_COLUMN_AREA_HEIGHT) {
      ...
      return {
        text: renderFleetGrid(runs, {
          ...
          // CON-107: threaded through the same way quickStartFocus is —
          // renderFleetGrid's own METRICS box is the one call site that
          // actually renders the expanded tier's focused/windowed history.
          metricsEscalationFocus: opts && opts.metricsEscalationFocus,
...
function mergeRenderOpts(state, opts) {
  return Object.assign({}, opts, {
    ...
    // cursor — same "read straight off state" pattern as quickStartFocus
    // just above.
    metricsEscalationFocus: state.metricsEscalationFocus,
