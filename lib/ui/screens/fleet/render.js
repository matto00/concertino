  ...
  sections.forEach((s, i) => {
    ...
    for (let k = w.startOffset; k < w.startOffset + w.shown; k++) {
      ...
      if (s.kind === 'queued') {
        ...
        for (const line of renderQuickStartRow(ticket, focused, innerCols)) { contentLines.push(line); contentLineOwners.push(null); }
      } else if (s.kind === 'failed' || s.kind === 'done') {
        const rowIndex = sectionStartIndex + k;
        for (const line of renderFinishedRow(s.group[k], { cols: innerCols, avgDoneMs }, rowIndex === selected)) { contentLines.push(line); contentLineOwners.push(rowIndex); }
      } else {
        const rowIndex = sectionStartIndex + k;
        for (const line of renderRun(s.group[k], { cols: innerCols, avgDoneMs }, rowIndex === selected)) { contentLines.push(line); contentLineOwners.push(rowIndex); }
      }
    }
