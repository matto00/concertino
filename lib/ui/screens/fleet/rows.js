...
function renderFinishedRow(run, opts, selected) {
  ...
  // below).
  //
  // CON-114, design.md Risks: a DONE row's own "marked for comparison"
  // indicator (`opts.compareSelected`, threaded from `state.compareSelection`
  // — render.js's per-row call site) reuses this exact same marker slot/
  // glyph rather than introducing a second one — FAILED's multiSelected and
  // DONE's compareSelected can never both be true for the same row (a row
  // is exactly one status), so there is no ambiguity about which selection
  // a rendered `✓` refers to.
  const multiMarker = (opts.multiSelected || opts.compareSelected) ? '✓' : ' ';
  const name = run.changeName || f.dim('(no branch yet)');
