...
function handleKey(key, state) {
  ...
  // cursor — read straight off `state`, exactly like `quickStartFocus` just
  // above (meaningful only while `focus === 'metrics'`).
  const metricsEscalationFocus = state && state.metricsEscalationFocus;
  ...
  // CON-114, design.md Decision 4: opens the compare screen once exactly
  // two DONE runs are marked for comparison — positioned immediately after
  // the restored-queue confirm check just above so a pending restored-queue
  // confirmation keeps its existing precedence over this new binding for
  // the same key (tasks.md 3.1). Fewer than two marked: falls through as a
  // no-op, exactly like `c` already does today when neither condition
  // applies.
  if (key === CONFIRM_RESTORED_QUEUE_KEY) {
    const compareSelection = (state && state.compareSelection) || [];
    if (compareSelection.length === 2) return { type: 'open-compare' };
  }
  ...
  if (key.length === 1 && key >= '1' && key <= '9') {
    ...
    switch (target.section.kind) {
      ...
      // CON-107: METRICS is now a genuine digit-jump target — its own
      // escalation-history cursor, mirroring QUICK START's own
      // 'focus-quickstart' just above.
      case 'metrics': return { type: 'focus-metrics', index: 0 };
  ...
  // CON-107, design.md Decision 2: while focus is on METRICS, j/k/Escape
  // reinterpret against the METRICS-local escalation-history cursor instead
  // of the ordinary run selection — sibling to the `focus === 'quickstart'`
  // block just above, only for the full raised/resolved escalation history
  // metricsFor() now computes rather than an eligible-ticket list. Unlike
  // QUICK START, `↵` is NOT suppressed here — it is METRICS' own "open the
  // selected entry" binding, emitted UNCONDITIONALLY with the unresolved
  // local cursor index (mirrors `quickstart-add`'s own "handleKey has no
  // history array to check against" discipline — the fleet controller's
  // 'open-historical-escalation' handler is the one place that actually
  // resolves it). `l`/arrow-right/`n`/`N` are suppressed, same as the
  // quickstart block.
  if (focus === 'metrics') {
    if (key === 'j' || key === '\x1b[B') return { type: 'move-metrics-focus', delta: 1 };
    if (key === 'k' || key === '\x1b[A') return { type: 'move-metrics-focus', delta: -1 };
    if (key === '\r') return { type: 'open-historical-escalation', index: metricsEscalationFocus };
    if (key === '\x1b') return { type: 'exit-metrics-focus' };          // bare Escape only
    if (key === 'l' || key === '\x1b[C' || key === 'n' || key === 'N') return null;
  }
  ...
  // side-by-side comparison — the identical binding site/guard as the
  // FAILED multi-select toggle just above, extended to the other status
  // this key now has meaning for (design.md: "space is unbound today for a
  // DONE-status row in that same guard"). Dispatches a different action
  // (toggle-compare-select, not toggle-multi-select) since a DONE row's
  // marking is the capped-at-2 comparison selection, not a FAILED/QUEUED
  // bulk-action set.
  if (key === ' ' && focus === 'runs' && runs[selected] && runs[selected].status === 'done') {
    return { type: 'toggle-compare-select', ticket: runs[selected].ticket };
  }
