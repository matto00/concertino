...
function renderFleetGrid(runs, ctx) {
  const {
    ...
    // CON-107, design.md Decisions 2-3: the METRICS-local escalation-history
    // cursor, threaded through render.js's own grid-mode ctx build exactly
    // like quickStartFocus just above.
    metricsEscalationFocus,
  ...
  if (metricsSection) {
    ...
    // CON-107, design.md Decisions 2-3: the ONLY call site where the
    // expanded tier's focused/windowed history actually renders — the
    // single-column path's own metricsColumnLines call (sections.js) never
    // reaches the expanded tier at all (its `contentRows` always defaults to
    // 5 — see that call site's own comment).
    const content = metricsColumnLines(metrics, {
      cols: metricsInner, contentRows: metricsContentRows,
      focused: focus === 'metrics', selectedIndex: metricsEscalationFocus,
    });
