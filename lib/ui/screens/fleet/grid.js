...
function renderStackedSection(s, jumpNumber, w, ctx) {
  ...
  for (let k = w.startOffset; k < w.startOffset + w.shown; k++) {
    ...
    if (s.kind === 'queued') {
      ...
      for (const line of renderFinishedRow(s.group[k], { cols: innerCols, avgDoneMs: ctx.avgDoneMs, searchQuery: ctx.searchQuery }, rowIndex === ctx.selected)) contentLines.push(line);
    } else {
...
function renderFleetGrid(runs, ctx) {
  ...
  const rowCtx = { avgDoneMs, selected, queuedTitles, queueFocus, quickStartFocus, focus, queueLaunchInfo, searchQuery };
  let jumpNumber = 0;
