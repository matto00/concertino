...
// [QUICK START if visible], [QUEUED if non-empty], DONE. Section order is
...
function buildSections(buckets, queueState, opts) {
  ...
  if (o.metrics) {
    ...
    // fitSegments call, alongside the success-rate bars) deliberately: it
    // used to always be visible before this METRICS expansion, and packing
    // it as line 2's LAST segment made it the first thing fitSegments drops
    // — at a completely standard 80-column terminal, line 2 needs ~95
    // visible columns for all three segments but only has ~76 available, so
    // one of the more actionable numbers in the box was silently vanishing
    // at the default width. Line 1 has no fitSegments call and so never
    // drops anything.
    ...
    // Sort by GATE_NAME_ORDER's display-order hint; anything gateRates
    // reports that GATE_NAME_ORDER doesn't know about (a new gate name
    // introduced later) sorts alphabetically after every known name, rather
    // than being dropped — see GATE_NAME_ORDER's own header comment.
...
function renderFleet(runs, opts) {
  ...
  // quickStartVisible/quickStartTickets already established.
...
// START.
//
// `metricsVisible` (lazygit-layout pass) is the fourth parameter, same
// reasoning — `handleKey`'s own call site always passes `true` (METRICS is
// unconditional, unlike QUICK START), but this function only needs to know
// whether the section is INCLUDED, not what it actually says, so a bare `{}`
// stand-in is enough; buildSections' METRICS branch only checks `o.metrics`
// for truthiness.
...
function handleKey(key, state) {
  ...
  if (key.length === 1 && key >= '1' && key <= '9') {
    ...
    // other way (tasks.md 2.9/3.3). METRICS (lazygit-layout pass) is
    // unconditional — always `true` here, unlike quickStartVisible.
