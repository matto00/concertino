...
for (const m of [
  ...
  './fleet/grid', './fleet/render', './fleet/keys', './fleet/search',
]) {
...
const { sectionJumpTargets, searchKey, handleKey } = require('./fleet/keys');
// CON-110: the search module's own exports, re-exported here alongside every
// other fleet/*.js submodule's — see this file's own header comment on why
// this facade stays the ONE module every consumer resolves.
const { matchesQuery, rowMatches, searchTargets, firstMatch } = require('./fleet/search');
...
module.exports = {
  ...
  searchKey, matchesQuery, rowMatches, searchTargets, firstMatch,
};
