...
// all already claimed — see handleKey below. CON-39 later claims digit keys
// 1-9 (section jump) and, while focus is on QUEUED, `f` (force-start) and
// bare Escape (exit queue focus) — none of these collide with `c` either.
...
function buildHeadTail(runs, opts) {
  ...
  if (forceStartConfirm) {
    ...
    if (queueState && queueState.pending && queueState.pending.length) hints.push('f force-start');
...
function sectionJumpTargets(runs, queueState) {
  const sections = buildSections(bucketRuns(runs), queueState).filter((s) => s.group.length > 0);
  let index = 0;
...
function handleKey(key, state) {
  ...
  // 'focus-queue' instead, since it has no slot in that index space at all.
  // Out of range (more digits than sections currently on screen) is a
  // no-op, identical to any other unbound key. Handled uniformly regardless
  // of `focus`, so pressing a different section's digit while focus is
  // 'queue' exits queue-focus and jumps as normal (tasks.md 4.3).
  if (key.length === 1 && key >= '1' && key <= '9') {
    const targets = sectionJumpTargets(runs, queueState);
    const n = parseInt(key, 10) - 1;
    if (n < 0 || n >= targets.length) return null;
    const target = targets[n];
    if (target.section.unselectable) return { type: 'focus-queue', index: 0 };
    return { type: 'jump', index: target.startIndex };
  }
