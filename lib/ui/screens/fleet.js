...
// START, [QUEUED if non-empty], FAILED, DONE. Section order is
...
function buildHeadTail(runs, opts) {
  ...
  if (clearQueueConfirm) {
    ...
    hints.push('n new run', 'N launch pad', 'q quit');
...
function visibleWindow(runs, opts) {
  ...
  // this call actually learns `quickStartTickets` and sizes a QUICK START
  // entry into the height budget below (design.md Decision 4, "none of
  // buildSections' three call sites forward opts", point 1; tasks.md 2.10).
  // Without this, QUICK START's row/height cost
