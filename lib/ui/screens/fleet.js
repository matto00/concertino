...
function sectionNaturalHeight(s, shown, groupLen) {
...
function computeWindow(runs, sections, opts) {
  ...
  const sectionHeight = (s, w) => sectionNaturalHeight(s, w.shown, s.group.length);
...
function visibleWindowGrid(runs, opts) {
  ...
  const needsYouHeight = needsYouSection ? sectionNaturalHeight(needsYouSection, needsYouShown, needsYouSection.group.length) : 0;
  const failedHeight = failedSection ? sectionNaturalHeight(failedSection, failedShown, failedSection.group.length) : 0;
  ...
  // N more" line (or nothing at all, for a forceRender-empty section, per
  // its own group.length-first branch) — while still reusing columnWindow's
  // own maxScrollOffset below, unaffected by this either way (a structural
  // property of `runs`/MAX_FINISHED alone, per computeWindow's own
  // contract).
