...
function buildHeadTail(runs, opts) {
  ...
  if (forceStartConfirm) {
    ...
    // CON-40: unlike `f force-start` just above (advertised only when a
    // QUEUED section is actually on screen), `Q` is a plain toggle that
    // always does something — open the panel if closed, close it if open —
    // so it is advertised unconditionally, matching `n`/`N`'s own
    // unconditional hints (design.md/tasks.md 3.6).
...
function renderFleet(runs, opts) {
  ...
  // correct elsewhere.
  const sections = buildSections(bucketRuns(runs), queueState, opts);
...
module.exports = {
  ...
  CONFIRM_RESTORED_QUEUE_KEY, visibleWindow, sectionJumpTargets, buildSections,
