...
function buildSections(buckets, queueState, opts) {
  ...
  // history) — reuses the exact same forceRender/emptyHint mechanism QUICK
  // START established above (a fixed, always-shown single summary line, no
  // selectable rows) rather than inventing a second "always render an empty
  // section" path. `o.metrics` is null/undefined only in the two call sites
