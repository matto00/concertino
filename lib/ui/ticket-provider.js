...
// withDefaults() already rewrites it, but withDefaults is NOT the only path a
// config reaches the dashboard by: lib/cli/watch.js's cmdWatch JSON.parses
// concertino.config.json straight off disk and hands the raw object to
// watch(). So the alias is resolved HERE too, at the one seam every provider
// consumer goes through, rather than trusting that some earlier loader
// normalised it — otherwise `concertino validate`'s "reads as local" promise
// is true of the agent half and false of the dashboard.
