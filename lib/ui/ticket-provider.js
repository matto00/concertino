...
// Object.create(null), not `{}` — a hand-written `ticketProvider.kind` of
// `constructor`/`toString`/`hasOwnProperty` must miss here and fall through
// to moduleFor()'s loud throw below, not silently resolve to an inherited
// Object.prototype member. Neither this object nor ALIASES below is ever
// called AS an object (no `.toString()`/`.hasOwnProperty()` on the table
// itself is used anywhere in this file) — only indexed — so the prototype
// chain buys nothing and only widens the lookup surface.
const MODULES = Object.assign(Object.create(null), { linear, local });
...
// Object.create(null) for the same reason as MODULES above — a hand-written
// `constructor`/`toString`/`hasOwnProperty` `kind` must miss this table too,
// not resolve to an inherited member and silently masquerade as an alias.
const ALIASES = Object.assign(Object.create(null), { manual: 'local' });
...
function moduleFor(config) {
  ...
  //
  // Kept terse so a typical (short) offending kind is still VISIBLE rather
  // than truncated away by launchpad.js's single-line f.truncate(msg,
  // cols - 4) on an 80-column terminal (76 columns of budget there — see
  // test/ticket-provider.test.js's own truncation-budget test, which
  // exercises both a short kind that fits and a long one that does not).
  // That is the whole diagnostic value of the message, so the terser "not X"
  // wins over local.js's longer "this project uses X" phrasing.
  if (!mod) {
...
// CON-93 item 4: `concertino validate --ticket <ID>`'s single-ticket fetch,
// mirroring fetchTickets' dispatch shape above. `opts` is
// { id, apiKey, root, transport }; linear ignores `root`, local ignores
// `apiKey`/`transport`. Both branches are awaitable: linear.fetchOneTicket is
// already async, local.fetchOneTicket returns a Promise itself (it never
// throws synchronously), so no extra Promise.resolve wrapping is needed here.
function fetchOneTicket(config, opts) {
  const mod = moduleFor(config);
  if (mod === linear) return linear.fetchOneTicket(opts);
  return mod.fetchOneTicket(opts);
}
