...
// CON-179: a `subAnswers` entry is either the bare value (the shape every
// slot has always had) or `{ question, value }` once a caller supplies the
// sub-question's own text — see writeSubAnswer's `question` param. Reading
// back "the value regardless of shape" is needed in more than one place
// (completeness/emptiness checks here, and the resolve loop in
// emit-event.sh has its own copy of this exact normalization in JS), so it
// is factored out once here rather than re-derived ad hoc.
function subAnswerValue(entry) {
  return (entry && typeof entry === 'object' && !Array.isArray(entry) && 'value' in entry) ? entry.value : entry;
}

function subAnswerQuestion(entry) {
  return (entry && typeof entry === 'object' && !Array.isArray(entry) && 'question' in entry) ? entry.question : null;
}

// CON-179 (cycle-2 review, finding 4): once a provenance-mismatched
// answer.json reaches `complete: true`, every slot is non-null — so every
// future writeSubAnswer() call for any index hits the "already answered"
// refusal below, forever. emit-event.sh's own resolve loop treats a
// mismatch as non-terminal (keeps polling, never resolves it), but nothing
// on the WRITE side could ever correct it: not the dashboard, not the CLI,
// only hand-deleting answer.json. Pre-CON-179 this at least resolved
// (wrongly, but resolvably) — CON-179 must not make a bad file permanently
// stuck.
//
// This mirrors the exact check emit-event.sh's try_resolve() runs (see its
// header comment for the full rationale) so "does the write side agree this
// file is stale" and "does the read side agree it is malformed" never
// diverge: an entry with no recorded `question` (the legacy shape) is never
// itself CLASSIFIED as mismatched (there is nothing in it to compare against
// sub_questions) — but it is not exempt from the discard that follows a
// mismatch found elsewhere in the same file: the whole file is discarded as
// one unit (see writeSubAnswer below), so a legacy entry sitting alongside a
// mismatched one is wiped as collateral, exactly like every other
// already-recorded answer in that file. An entry whose recorded `question`
// disagrees with the CURRENT sub_questions[index] means this file was
// answered against a different, or differently-ordered, raise of this
// escalation.
function hasProvenanceMismatch(subAnswers, subQuestions) {
  if (!Array.isArray(subQuestions)) return false; // nothing to check against
  return subAnswers.some((entry, i) => {
    const q = subAnswerQuestion(entry);
    if (q == null) return false;
    const expected = subQuestions[i] && subQuestions[i].question;
    return expected != null && q !== expected;
  });
}

// `question` is optional (and additive — every existing caller that omits it
// keeps writing the exact bare-value shape it always has). When supplied, it
// is the sub-question's own text (as raised — see lib/cli/answer.js's
// readEscalationShape and controllers/escalation.js's answer-sub dispatch,
// both of which read it from the SAME escalation.raised event this
// escalation's `total` was itself derived from). Recording it here is what
// lets emit-event.sh's resolve loop detect a stored-text-vs-currently-raised
// mismatch (CON-151's silent positional misattribution) instead of trusting
// the array's index alone.
//
// `subQuestions` (optional, additive) is the full raised sub-question array
// (same source `question` itself comes from) — passing it lets a call here
// recover from a provenance-mismatched file (see hasProvenanceMismatch
// above) by discarding it and starting fresh, instead of refusing every
// slot as "already answered" forever.
function writeSubAnswer(root, ticket, index, value, total, question, subQuestions) {
  const dir = runDir(root, ticket);
  const target = answerPath(root, ticket);
  try {
    ...
    const arityMatches = existing && Array.isArray(existing.subAnswers) && existing.subAnswers.length === total;
    const mismatched = arityMatches && hasProvenanceMismatch(existing.subAnswers, subQuestions);
    // CON-179 (cycle-3 review, finding 3): a discard must not be silent. The
    // whole file is discarded as one unit on a mismatch (never partially —
    // there is no reliable way to tell which OTHER slots are still trustworthy
    // once one is proven to belong to a different raise), which can destroy
    // real, previously-recorded answers as collateral, including a legacy
    // entry that was never itself mismatched (see hasProvenanceMismatch's own
    // comment above). Count what is actually being thrown away so a caller
    // can tell the human, rather than resetting quietly.
    const discardedCount = mismatched
      ? existing.subAnswers.filter((a) => subAnswerValue(a) != null).length
      : 0;
    const subAnswers = (arityMatches && !mismatched)
    ...
    subAnswers[index] = (question != null) ? { question, value } : value;
    const complete = subAnswers.length === total && subAnswers.every((a) => subAnswerValue(a) != null);
    ...
    return discardedCount > 0
      ? { ok: true, complete, discarded: true, discardedCount }
      : { ok: true, complete };
  } catch (e) {
    return {
      ok: false,
...
module.exports = {
  ...
  writeAnswer, writeSubAnswer, readSubAnswers, subAnswerValue, createEventsCache,
};
