...
// diverge: an entry with no recorded `question` (the legacy shape) has
// nothing to check and is never treated as mismatched; an entry whose
// recorded `question` disagrees with the CURRENT sub_questions[index] means
// this file was answered against a different, or differently-ordered, raise
// of this escalation.
...
function writeSubAnswer(root, ticket, index, value, total, question, subQuestions) {
  ...
  try {
    ...
    const reusable = existing && Array.isArray(existing.subAnswers) && existing.subAnswers.length === total
      && !hasProvenanceMismatch(existing.subAnswers, subQuestions);
    const subAnswers = reusable
