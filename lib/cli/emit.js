...
function emitCodex(c, out, core, dry) {
  ...
  // CON-65: gated on providers.ollama EXISTING, not on codex being listed in
  // providers.ollama.harnesses — the [model_providers.ollama] block is inert
  // wiring (nothing routes through it until a per-agent `model_provider` line
  // or a per-run `-c model_provider=ollama` override points at it), and
  // rendering it whenever the provider is configured is what makes a
  // per-ticket `provider:ollama` flip work on a project whose codex default
  // is subscription (see lib/ui/harness.js's providerCommandFlags).
