...
// CON-191 design.md Decision 3/4: the render-time digest baseline against
// which setup-worktree.sh's own live digest is compared at run start. Same
// algorithm as that script's compute_instrumentation_manifest() (per-file
// sha256, aggregated over sorted RELATIVE-PATH:hash pairs) — computed here
// in JS rather than shelling out to node a second time, since this code
// already runs under node.
//
// CON-191 skeptic-final-1 (round 1, REFUTE): the original version of this
// function walked `scriptsDir` NON-recursively (`fs.readdirSync(scriptsDir)`,
// top-level only), which silently excluded `scripts/concertino/lib/**` —
// `auditor-lease.sh`, `git-child-env.sh`, `pr-reconcile.sh`,
// `run-end-status.sh`, etc. — even though `emit-event.sh` itself `source`s
// `lib/auditor-lease.sh` on every invocation, so that file's content is
// directly part of what executes. A patch to a nested `lib/*.sh` file moved
// the digest not at all — precisely the CON-128 failure shape (a local
// rendered-copy patch invisible to detection) this capability exists to
// catch. Fixed by using listFilesRecursive() (already shared with
// copyAssets() below) so every `.sh` file anywhere under `scriptsDir` is
// hashed, keyed and sorted by its full, forward-slash-normalized RELATIVE
// path (e.g. `lib/auditor-lease.sh`) — never a bare basename, which could
// collide across directories or sort inconsistently. `core/`'s own
// templates are still never read here (the whole point is to record what a
// consuming repo will actually execute).
function computeInstrumentationManifest(scriptsDir) {
  const crypto = require('crypto');
  const files = listFilesRecursive(scriptsDir).filter((f) => f.endsWith('.sh')).sort();
  const fileHashes = {};
  const agg = crypto.createHash('sha256');
  for (const f of files) {
    const content = fs.readFileSync(path.join(scriptsDir, f));
    const h = crypto.createHash('sha256').update(content).digest('hex');
    fileHashes[f] = h;
    agg.update(f + ':' + h + '\n');
  }
  return { digest: agg.digest('hex'), files: fileHashes };
}
...
function copyAssets(out, core, dry, withScripts) {
  ...
  if (withScripts) {
    ...
    if (!dry) {
      const scriptsDir = path.join(out, 'scripts', 'concertino');
      const manifest = computeInstrumentationManifest(scriptsDir);
      fs.writeFileSync(
        path.join(scriptsDir, '.instrumentation-manifest.json'),
