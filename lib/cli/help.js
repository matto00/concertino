...
const USAGE = {
  ...
  answer: `  ${cyan('concertino answer')} ${dim('<ticket> <value> [--sub <index> [--total <n>]] [--out=DIR]')}
      ...
      For a multi-part escalation, ${dim('--sub <index>')} is 1-based (the
      first sub-question is ${dim('--sub 1')}), matching the dashboard
      wizard's "sub-question N of total" display and this command's own
      "(sub N/total)" confirmation. An out-of-range index (< 1 or > --total)
      is rejected before anything is written.
      The real sub-question count is read from the ticket's own
      ${dim('escalation.raised')} event whenever one exists: ${dim('--sub')}
      with no ${dim('--total')} derives it automatically, and a ${dim('--total')}
      that does not match the real count is refused outright (never silently
      honoured — a wrong total would otherwise resize ${dim('answer.json')}
      and could drop already-recorded answers). Answering a real multi-part
      escalation with no ${dim('--sub')} at all is refused, not silently
      recorded as a single-question answer. ${dim('--total')} stays required
      only when no ${dim('escalation.raised')} event exists to check against.`,
  ...
  report: `  ${cyan('concertino report')} ${dim('<name> [--config=PATH] [--out=DIR]')}
      Run a read-only, on-demand report. Never writes, mutates, or emits
      an event.
      Known reports:
        ${dim('followup-survival')}   Fraction of origin_kind=followup tickets
                              reaching a completed state, segmented by
                              origin_role/suggested_by/triage.overlap and
                              bucketed by filing week. Two tiers — provenance
                              (ticket.filed events, authoritative) and
                              recovered-heuristic (text matching, indicative
                              only) — are always reported separately, never
                              summed. Requires ${dim('LINEAR_API_KEY')}.
      Example: ${dim('concertino report followup-survival')}`,
};
...
const USAGE_ORDER = [
  ...
  'upgrade', 'gates', 'eject', 'migrate', 'answer', 'completion', 'report',
];
