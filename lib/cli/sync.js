...
const { REPO, dim, green, red, write, resolveOut, resolveConfigPath, hasHelpFlag, parseHarnessList, reportProvenance } = require('./shared');
...
async function cmdSync(args, resolvedCore) {
  ...
  if (dry) console.log(dim('  (dry run: filenames only — run `concertino diff` for a content-level preview)'));
