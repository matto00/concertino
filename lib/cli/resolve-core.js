...
const { exists, yellow, listFilesRecursive, gitRun, gitTopLevel } = require('./shared');
...
function coresDiffer(coreA, coreB) {
  ...
  // 'scripts' alone uses the recursive listing — core/scripts/lib/ is a real
  // nested subdirectory (see core/scripts/lib/git-child-env.sh), and
  // fileDiffers() below has no try/catch around its readFileSync, unlike
  // readDirSafe, so a bare readDirSafe-based enumeration would throw EISDIR
  // comparing a directory entry as if it were a file. 'laws'/'roles' have no
  // nested directories today, so they stay on the flat readDirSafe.
  const scriptNames = new Set([
    ...listFilesRecursive(path.join(coreA, 'scripts')),
    ...listFilesRecursive(path.join(coreB, 'scripts')),
  ]);
  for (const f of scriptNames) {
    if (fileDiffers(path.join(coreA, 'scripts', f), path.join(coreB, 'scripts', f))) return true;
  }
  for (const sub of ['laws', 'roles']) {
