...
const { red, hasHelpFlag } = require('./shared');
const { printUsage } = require('./help');

function cmdCompletion(args) {
  if (hasHelpFlag(args)) { printUsage('completion'); return; }
  const shell = args._[1] || 'fish';
  const CMDS = ['init', 'sync', 'update', 'validate', 'diff', 'doctor', 'watch', 'prune', 'upgrade', 'gates', 'completion', 'eject', 'migrate', 'answer', 'help'];
  const DESC = { init:'Interactive setup', sync:'Render harness files', update:'Update config fields and re-sync', validate:'Check config for errors', diff:'Show sync diff vs disk', doctor:'Check environment health', watch:'Live fleet dashboard', prune:'Remove old event logs', upgrade:'Detect stale generated files', gates:'List or run a gate', completion:'Print shell completion script', eject:'Print a rendered agent file', migrate:'Back-fill missing config fields', answer:'Answer a pending escalation', help:'Show help' };
  ...
  if (shell === 'fish') {
    const lines = [
      ...
      'complete -c concertino -n "__fish_seen_subcommand_from prune" -l dry-run -d "Preview without writing"',
      'complete -c concertino -n "__fish_seen_subcommand_from eject" -l role -r -a "orchestrator executor evaluator skeptic auditor" -d "Agent role"',
      'complete -c concertino -n "__fish_seen_subcommand_from eject" -l harness -r -a "claude-code codex opencode" -d "Target harness"',
      'complete -c concertino -n "__fish_seen_subcommand_from migrate" -l dry-run -d "Preview without writing"',
      'complete -c concertino -n "__fish_seen_subcommand_from answer" -l sub -r -d "Sub-answer index (1-based)"',
      'complete -c concertino -n "__fish_seen_subcommand_from answer" -l total -r -d "Sub-answer total"',
    ];
    ...
    const args_map = {
      ...
      'watch':       '"--out=[project root]:dir:_files -/" "--config=[config path]:file:_files"',
      'prune':       '"--dry-run[preview without writing]"',
      'eject':       '"--role=[agent role]:role:(orchestrator executor evaluator skeptic auditor)" "--harness=[target harness]:harness:(claude-code codex opencode)"',
      'migrate':     '"--dry-run[preview without writing]"',
      'answer':      '"--sub=[sub-answer index]:index:" "--total=[sub-answer total]:total:"',
    };
    const lines = [
      '#compdef concertino',
    ...
    const lines = [
      ...
      '    --run)      COMPREPLY=() ;;',
      '    --role)     COMPREPLY=($(compgen -W "orchestrator executor evaluator skeptic auditor" -- "$cur")) ;;',
      '    --sub|--total) COMPREPLY=() ;;',
      '    completion) COMPREPLY=($(compgen -W "fish zsh bash" -- "$cur")) ;;',
      '    *)          COMPREPLY=($(compgen -W "--out --config --dry-run --harness --run --yes --example --role --sub --total" -- "$cur")) ;;',
      '  esac; fi',
      '}',
