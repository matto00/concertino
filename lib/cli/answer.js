...
function readEscalationShape(root, ticket) {
  ...
  if (!last) return { found: false, multiPart: false, total: null, escalationId: null };
  let total = 0;
  let subQuestions = [];
  ...
  // CON-188 task 4.1: the raise's escalation_id, read back the same way
  // emit-event.sh's own read_raised_field() does — from the LAST
  // escalation.raised event for this ticket, never from caller input. Absent
  // (null) on any pre-CON-188 raise, which recordAnswered() below already
  // tolerates (design.md Decision 3 — omit the field rather than fabricate one).
  const escalationId = last.escalation_id != null ? last.escalation_id : null;
  return { found: true, multiPart: total > 0, total, subQuestions, escalationId };
}
...
async function cmdAnswer(argv) {
  ...
  if (!ticket || value === undefined) {
    console.error(red('error: ') + 'usage: concertino answer <ticket> <value> [--sub <index> --total <n>] [--channel=<dashboard|cli|chat|self-approved>] [--out=DIR]');
    process.exit(1);
  }

  // CON-188 task 4.2/design.md Decision 7: an explicit channel selector,
  // defaulting to `cli` so every existing invocation keeps working unchanged
  // and correctly labelled. Refused BEFORE any answer.json or event write —
  // a typo must never silently record a valid answer under a bogus channel.
  const LEGAL_CHANNELS = ['dashboard', 'cli', 'chat', 'self-approved'];
  const channel = flags.channel === undefined ? 'cli' : flags.channel;
  if (!LEGAL_CHANNELS.includes(channel)) {
    console.error(red('error: ') + 'invalid --channel ' + JSON.stringify(channel)
      + ' (must be one of: ' + LEGAL_CHANNELS.join(', ') + ')');
    process.exit(1);
  }
  // design.md Decision 6: answer_source is derived from the channel, never
  // supplied independently — dashboard/cli/chat imply human, self-approved
  // implies agent-default.
  const answerSource = channel === 'self-approved' ? 'agent-default' : 'human';
  ...
  if (hasSub) {
    ...
    // CON-179 (cycle-3 review, finding 3): a discard must not be silent --
    // this write may have just thrown away previously-recorded answers (a
    // provenance mismatch discards the WHOLE file as one unit, never
    // partially). Tell the human before anything else this write's result
    // prints.
    if (result.discarded) {
      console.log(yellow('warning: ') + 'a previous answer.json for ' + ticket
        + ' did not match the currently-raised escalation and was discarded ('
        + result.discardedCount + ' prior answer' + (result.discardedCount === 1 ? '' : 's')
        + ' lost) — starting this escalation over from this answer');
    }
    ...
    if (result.complete) {
      ...
      // CON-188 task 4.3: resolution_channel/answer_source ride along on
      // every completing write; escalation_id only when the raise actually
      // carried one (design.md Decision 3 — omit, don't fabricate).
      const extraArgs = ['sub_answers=' + subAnswersJson, 'resolution_channel=' + channel, 'answer_source=' + answerSource];
      if (shape.escalationId) extraArgs.push('escalation_id=' + shape.escalationId);
      recordAnswered(root, ticket, extraArgs);
  ...
  // CON-188 task 4.3: same shape as the multi-part completion path above.
  const extraArgs = ['answer=' + value, 'resolution_channel=' + channel, 'answer_source=' + answerSource];
  if (shape.escalationId) extraArgs.push('escalation_id=' + shape.escalationId);
  recordAnswered(root, ticket, extraArgs);
