...
async function cmdAnswer(argv) {
  ...
  if (hasSub) {
    ...
    if (!Number.isInteger(humanIndex) || !Number.isInteger(total)) {
      console.error(red('error: ') + '--sub <index> (1-based) and --total <n> must both be integers');
      process.exit(1);
    }
    ...
    const result = store.writeSubAnswer(root, ticket, slot, value, total);
