...
const {
  ...
  section, read, exists, resolveOut, resolveConfigPath, hasHelpFlag,
} = require('./shared');
const { resolveCore } = require('./resolve-core');
const {
  checkAgentMergePermission, withAgentMergeFixHint,
...
// ---------- cost-tracking hook check (doctor, track-per-run-cost-spend) ---
// Runtime counterpart to `lib/config.js`'s `collectConfigIssues` "Cost
// tracking" section — same shared `checkCostTrackingHook`, reported through
// doctor's own ok/warn. Silent no-op unless costTracking.enabled is true and
// claude-code is a configured harness, mirroring checkAgentMerge above.
function checkCostTracking(out, cfg, r) {
  const harnesses = Array.isArray(cfg.harnesses) ? cfg.harnesses : [];
  if (!cfg.costTracking || !cfg.costTracking.enabled || !harnesses.includes('claude-code')) return;
  section('Cost tracking');
  const result = checkCostTrackingHook(cfg, out);
  if (result && result.ok) {
    r.ok('costTracking.hooks', 'SessionEnd + SubagentStop present in .claude/settings.json');
  } else {
    const reason = (result && result.reason) || 'hook entries missing';
    r.warn(withCostTrackingFixHint(reason));
  }
}

function cmdDoctor(args) {
  if (hasHelpFlag(args)) { printUsage('doctor'); return; }
  ...
  if (!exists(cfgPath)) {
    ...
    if (cfg) {
      ...
      checkCostTracking(out, cfg, { ok, warn });
