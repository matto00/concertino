...
const { VALID_HARNESSES, docsPlaceholderName } = require('../config');
...
function block(name, c, harness) {
  ...
  switch (name) {
    // CON-195: driven by the shared role->placeholder-name mapping
    // (docsPlaceholderName, lib/config.js) rather than independent literals,
    // so this switch and collectConfigIssues' placeholder-coverage check can
    // never disagree about what placeholder a role expects (design.md
    // Decision 2).
    case docsPlaceholderName('executor'): return docList('executor');
    case docsPlaceholderName('evaluator'): return docList('evaluator');
    case docsPlaceholderName('skeptic'): return docList('skeptic');
    case docsPlaceholderName('auditor'): return docList('auditor');
    case docsPlaceholderName('orchestrator'): return docList('orchestrator');
    case 'gates': {
      const groups = [];
    ...
    case 'specArtifacts': {
      if (sp.kind === 'openspec') {
        const validate = sp.validateCmd || 'openspec validate "<CHANGE_NAME>" --type change';
        return [
    ...
    case 'standaloneTicket': {
      // CON-190: after filing, mirror provenance onto the ticket itself and
      // emit `ticket.filed` recording it — see the `ticket-provenance` /
      // `followup-triage` capabilities. `origin_repo` is derived from the
      // repository the orchestrator is actually running in (never the
      // filed ticket's own id prefix — that conflation is what invalidated
      // the original provenance inference, per ticket.md).
      const provenanceEmit =
      ...
      const linearGithubWording =
        ...
        '     scope change to the current run.' + provenanceEmit;
      return {
        ...
        local: 'derive `<prefix>` from `$TICKET_ID` by stripping its trailing `-<digits>`\n' +
          ...
          '     frontmatter `title:` (a short title drawn from `description`),\n' +
          '     `state: backlog`, `origin_kind: followup`, and `origin_ticket: $TICKET_ID`\n' +
          '     (so provenance is queryable independently of the event log), and a body\n' +
          '     summarizing `description` plus a line linking back to the current ticket\n' +
          '     (`$TICKET_ID`); note the new ticket\'s identifier in your summary to the\n' +
          '     human. No re-planning, no scope change to the current run.' + provenanceEmit
      }[c.ticketProvider.kind] || '';
    }
    ...
    case 'subagentEscalationNotify':
      // CON-127: the raiser's fire-and-forget durable self-notify to the
      // orchestrator, immediately before returning an ESCALATION/
      // ESCALATION-RAISE verdict. Claude-Code-only (this is the only harness
      // where a sub-agent holds a tool that can address its spawner at all) —
      // codex/opencode get an empty string so the shared role-doc prose that
      // references `{{block:subagentEscalationNotify}}` collapses to nothing
      // there, keeping the zero-SendMessage-occurrence-delta guarantee.
      if (harness === 'codex' || harness === 'opencode') return '';
      return "Before returning, self-notify the orchestrator: call `SendMessage` targeting `ORCHESTRATOR_AGENT_REF` (given to you at spawn/resume time) with your `Question`/`Options`/`Context`. This is fire-and-forget — do not wait for a reply, and do not loop or block on delivery. Send it, then return your escalation report exactly as below.";
    case 'harnessResume':
      if (harness === 'codex') {
...
function renderBody(text, c, harness) {
  ...
  // `<change-dir>` (e.g. `openspec/changes/<name>`) — see lib/config.js's
  // `specProvider.changeRoot` derivation.
  out = out.split('<change-dir-root>').join(c.specProvider.changeRoot);
...
function renderEnv(c) {
  ...
  // CON-148: only emitted when opted in, so a project that omits `cleanup`
  // gets a byte-identical .concertino.env to before this change. `cleanup.sh`
  // sources this file (line 127) before reading the variable (line 501), and
  // the value goes through the same truthy set the script accepts.
  if (c.cleanup && c.cleanup.skipSync === true) {
    L.push('CONCERTINO_CLEANUP_SKIP_SYNC=1');
  }
