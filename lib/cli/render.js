...
function renderSpeedsJson(c) {
  ...
  // claude-code always excluded from provider-model substitution (the
  // gateway remaps hosted aliases; see isOllamaRouted's comment in
  // lib/config.js) — honoring CONCERTINO_PROVIDER as the per-run override
  // and `providers.ollama.harnesses` as the project default. Same output as
  // the fold for every pre-existing call path; newly flippable per window.
  const speeds = {
    budgets: c.budgets,
    speeds: c.speeds,
