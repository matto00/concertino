...
This is the one pattern in this repo that isn't obvious from directory names alone, and it has already caused a real bug — read this before editing anything under `scripts/concertino/`, `.claude/`, `.codex/`, `.cursor/`, or `.opencode/`.
...
**`concertino sync`** (`bin/concertino`) renders `core/` + your project's `concertino.config.json` into each configured harness's native layout: `scripts/concertino/*.sh` (copied byte-for-byte from `core/scripts/`), `.claude/agents/*.md` + `.claude-plugin/` (Claude Code), `.codex/` (Codex), `.cursor/`/`.opencode/` mirrors where applicable, and `.concertino/laws/` + `.concertino/workflow-state.template.md`.
